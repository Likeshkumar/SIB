...
<s:form action="addonCardActionAddonCardAction.do"  name="orderform" autocomplete = "off"  namespace="/">
	...
	<table border="0" cellpadding="0" cellspacing="4" width="50%" class="formtable">
				...
				<tr id="cardnumdiv" >
					<td>Enter Card No. :</td>
					<td><s:textfield name="cardnum" id="cardnum" maxlength="19" onkeypress='return isNumberKey(event)' value="%{bean.cardno}"/></td>
				</tr>
...
<%-- 	<%String act = request.getParameter("act")==null?"":request.getParameter("act"); %> --%>
	...
	<input type="hidden" id="cardnumber" name="cardnumber" value="${hcardNumber}"/>
	</s:if>
	<s:else>
	...
	<input type="hidden" id="maskedcardno" name="maskedcardno" value="${MCARD_NO}"/>
							...
							<tr> 
								...
								<td style="text-align:center">${EMB_NAME}</td>
	...
	<s:if test='padssenabled=="Y"' >
	<tr>
	<td style="text-align:center">RefernceId :<input type="text" id="refId" name="refId"/></td>
	 <td style="text-align:center">Emb NAME :<input type="text" id="addonname" name="addonname"/></td>								
	 <td style="text-align: center"> SUB PRODUCT_CODE :
					<select id="subprodId"	name="subprodId">
							<option value='-1'>Select</option>
							<s:iterator value="subProdList">
								<option value='${SUB_PROD_ID}'>${SUB_PRODUCT_NAME}</option>
							</s:iterator>
					</select></td>
				<td style="text-align: center">Collection Branch</td>
					<td> <select name="cardcollectbranch" id="cardcollectbranch">
								<option value="">-SELECT-</option>
								<s:iterator value="cardcollectbranchlist">
									<option value="${BRANCH_CODE}">${BRANCH_NAME}</option>
								</s:iterator>
