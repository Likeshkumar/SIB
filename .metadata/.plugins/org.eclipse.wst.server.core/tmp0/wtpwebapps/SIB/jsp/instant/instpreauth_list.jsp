...
%>
<div align="center">
<s:form action="authourizeInstPREInstCardPREProcess"  name="orderform" onsubmit="return validateinstorder()" autocomplete = "off"  namespace="/">
	...
	<table width="90%" cellpadding="0"  border="0"  cellspacing="0"  class="formtable"  >
					...
					<tr>
						<th> Sl no </th>
					<th>  <input type='hidden' onclick="checkedAll(this.form)"  id="checkall"/> </th>
						<th> Order Ref No </th>
						<!-- <th>No.of Cards</th> -->
						<th> Card No </th>
						<th> Emb Name </th>
				...
				<s:iterator value="instorderlist">
					...
					>
						...
						<td> <input type="hidden" name="instorderrefnum"  id="orderrefnum<%=rowcount%>"  value="${ORDER_REF_NO}"/>  </td>
						<td> ${ORDER_REF_NO}  </td>
						<%-- <td> ${CARD_QUANTITY}  </td> --%>
						<td> ${MCARD_NO}</td>
						<td> ${EMB_NAME} </td>
			 		...
			 		<tr>
						<td colspan="8">
							<s:submit value="Submit" name="submit" id="submit" onclick="return selectallvalidationpre();"/>
