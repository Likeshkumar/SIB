		...
		<s:if test="#makerchek.mkckflag=='disabled'">
					...
					<s:iterator  value="#sunny.superAddProfSubmenuList"> 
						<s:set name="menuid"> ${MENUID} </s:set>					
								<tr> 
										<td>&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="mainmenu${mainid}" id="submenu${mainid}<%=submencnt++%>" value="${MENUID}" disabled="true"/>${MENUNAME}
										<input type="hidden" name="mainmenu${mainid}Sel${MENUID}" id="mainmenu${MENUID}" value="${MENUID}"/>
										</td>																					
								</tr>								
					</s:iterator>				
		</s:if>
