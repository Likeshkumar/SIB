...
<form
	<s:if test="%{statusflag=='edit'}">action="editdetailsAddSubProdAction.do"</s:if>
	<s:else>action="generateSubprodlistAddSubProdAction.do"</s:else>>
	...
	<table border="0" cellpadding="0" cellspacing="0" width="50%"
		...
		<tr>
			...
			<td>: <s:if test="enableauth">
					<select name="cardtype" id="cardtype"
						onchange="getSubProd( '<%= instid %>','<%=userid %>',this.value, '0' );">
				</s:if> <s:else>
					<select name="cardtype" id="cardtype"
						onchange="getSubProd( '<%= instid %>','<%=userid %>',this.value, '9' );">
				</s:else>
				<option value="-1">--Select --</option> <s:iterator value="prodlist">
		...
		<tr>
			<td align="right">SUB-PRODUCT</td>
			<td>: <select name="subproduct" id="subproductlist"
				onchange="maxAllowedCards('<%= instid %>', this.value)">
	...
	<table>
		<tr>
			<td><s:if test="enableauth">
					...
					<input type="submit" value="View & Authorize" name="auth" />
					<input type="hidden" name="next" id="next" value="viewauth">
				</s:if> <s:else>
					<!-- <input type="submit" value="View" name="view" onclick="return selectall();" /> -->
					<input type="submit" value="View" name="view" />
					<s:if test="%{statusflag=='edit'}">
						<input type="hidden" name="next" id="next" value="viewedit">
					</s:if>
					<s:else>
						<input type="hidden" name="next" id="next" value="viewauth">
					</s:else>
