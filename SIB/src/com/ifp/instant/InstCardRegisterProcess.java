...
import test.URLClientRegistration;
...
public class InstCardRegisterProcess extends BaseAction {
	 ...
	 * datasource.setUsername("dbbl_cbs"); datasource.setPassword("dbbl_cbs");
	 * return datasource; }
	 */
	...
	public String findreissucardsbin() {
		...
		/*
		 * if (comBranchId().equals("000")){ session.setAttribute("preverr",
		 * "E"); session.setAttribute("prevmsg",
		 * " Please Login With branch User ..." ); trace(
		 * "Please Login With branch User ..."); return "required_home"; }
		 */
	...
	public List getcustdetails(String instid, String chn, JdbcTemplate jdbctemplate) throws Exception {
		 ...
		 * " SELECT * from CARD_PRODUCTION where inst_id='"+instid+
		 * "' and CARD_NO='"+chn+"'"; enctrace( "getcarddetailscustomermap : " +
		 * custdetailsqry ); custdetail =
		 * jdbctemplate.queryForList(custdetailsqry);
		 */
	...
	public String ReissuCustReg() {
		...
		try {
			...
			System.out.println(branchid + "decrypted card number for encod and maintanance" + ecardno
					+ "clear chn value" + hcardno);
			...
			if (cardavailabe == 0 || card_no.length() != 16) {
				...
				session.setAttribute("prevmsg",
						" Entered CardNo Is " + card_no + ", Enter Correct Card Number/Card is not available ..");
			...
			if (validbranch == null) {
				...
				session.setAttribute("preverr", "E");
				session.setAttribute("prevmsg",
						" " + maskedchn + " Card is Available / Card Collect Branch not Available ..");
			...
			if (cardrequired == 2) {
				session.setAttribute("prevmsg",
						" " + maskedchn + "  Card is  Already Reissued With New Card Number.. ");
				session.setAttribute("preverr", "E");
			...
			if (cardrequired != 1) {
				session.setAttribute("prevmsg",
						" " + maskedchn + "  Card is  Active, Mark As Lost/Stolen For Mapping another card number");
				session.setAttribute("preverr", "E");
			...
			while (itrat.hasNext()) {
				...
				String productdesc = commondesc.getProductdesc(instid,
						(String) map.get("PRODUCT_CODE").toString().trim(), jdbctemplate);
				...
				// getjsonvalues = this.getcustdetails(instid,
				// cin,orderrefno,jdbctemplate);
			...
			setCardno(card_no);
			/*
			 * int regreq = cardregdao.checkcustomeridexist(instid,accountno,
			 * jdbctemplate); trace("Got regreq: " + regreq ); if( regreq > 0 ){
			 * //subproddesc = commondesc.getSubProductdesc(instid, subprodid,
			 * jdbctemplate); session.setAttribute("preverr", "E");
			 * session.setAttribute("prevmsg", "[ "+ accountno +
			 * " ] Account Already Exist. Registeration not required" ); trace(
			 * "[ "+ accountno +
			 * " ] Account Number Alredy Exist. Registeration not required");
			 * return "required_home"; }
			 */
	...
	public String SaveReissuCustomerReg() throws Exception {
		...
		if (getRequest().getParameter("orderrefno") != null) {
			...
			String validorderformaping = commondesc.getorderrefstaus(instid, getRequest().getParameter("orderrefno"),
					jdbctemplate);

			if ((validorderformaping == null) || (validorderformaping.equals(""))) {
				session.setAttribute("preverr", "E");
				session.setAttribute("prevmsg",
						"   Entered  Order Ref  Number is " + orderno + " ,Is Not Available... ");
				trace("Entered New  Order Ref Number is " + getRequest().getParameter("newcardno")
						+ " Waiting for the PinGeneration..");
				return "required_home";
			}
			...
			if (!(validorderformaping.equals("")) || (validorderformaping != null)) {
				...
				if (!listvalfornewcard.isEmpty()) {
					...
					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg",
							"   Entered  Order Ref  Number is " + orderno + " ,Waiting for the PinGeneration ");
					trace("Entered New  Order Ref Number is " + getRequest().getParameter("newcardno")
							+ " Waiting for the PinGeneration..");
					return "required_home";
					...
					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg", "   Entered Order Ref  Number is " + orderno
							+ " ,Waiting for the PinGeneration Authorization. ");
					trace("Entered New Card Number is " + getRequest().getParameter("newcardno")
							+ " Enter 16 digit Valid Card Number...");
					return "required_home";
				} else if (validorderformaping.equals("02P")) {

					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg",
							"   Entered Order Ref  Number is " + orderno + " ,Waiting for the PRE Generation. ");
					trace("Entered New Card Number is " + getRequest().getParameter("newcardno")
							+ " Enter 16 digit Valid Card Number...");
					return "required_home";
					...
					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg",
							"   Entered Order Ref  Number is " + orderno + " ,Waiting for the PRE Authorization. ");
					trace("Entered New Card Number is " + getRequest().getParameter("newcardno")
							+ " Enter 16 digit Valid Card Number...");
					return "required_home";
					...
					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg", "   Entered Order Ref  Number is " + orderno
							+ " ,Issued  To the Branch And Waiting for Authorization . ");
					trace("Entered Order Ref Number is " + getRequest().getParameter("newcardno")
							+ " Enter 16 digit Valid Card Number...");
					return "required_home";
				} else if (validorderformaping.equals("04P")) {
					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg",
							"   Entered Order Ref  Number is " + orderno + " ,Ready for the Issuance To Braches . . ");
					trace("Entered Order Ref Number is " + getRequest().getParameter("newcardno")
							+ " Enter 16 digit Valid Card Number...");
					return "required_home";
				}
				...
				if (!bin.equals("551270")) {
					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg",
							"   Entered  Order Ref  Number is " + orderno + " , not Master Card Bin");
				...
				// System.out.println("nehcardno::"+nehcardno+"newcardcollectbranch"+newcardcollectbranch);
					...
					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg",
							" " + orderno + "Order Number  is available & Card Collect Branch is not Available ...  ");
				...
				// comBranchId"+comBranchId());
					...
					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg",
							" " + orderno + " Order Number  is not Valid to map ..Card Collect Branch is '"
			...
			if (getRequest().getParameter("newcardno").length() != 16) {
				session.setAttribute("preverr", "E");
				session.setAttribute("prevmsg",
						"   Entered New Card Number is " + newcardno + " ,Enter 16 digit Valid Card Number... ");
				trace("Entered New Card Number is " + getRequest().getParameter("newcardno")
						+ " Enter 16 digit Valid Card Number...");
			...
			if (newcardcollectbranch == null) {
				session.setAttribute("preverr", "E");
				session.setAttribute("prevmsg",
						" " + newcardno + "Card is not available/Card Collect Branch is not configured ...  ");
			...
			if (!(newcardcollectbranch.equals(comBranchId()))) {
				...
				session.setAttribute("preverr", "E");
				session.setAttribute("prevmsg", " " + newcardno + " Card is not Valid to map ..Card Collect Branch is '"
						+ newcardcollectbranch + "' ");
		...
		/*
		 * getLimitType.append(
		 * "(select PRODUCT_CODE from INSTPROD_DETAILS where  SUB_PROD_ID='"
		 * +cardregbean.getCod_prod()+"' AND INST_ID='"+instid+
		 * "') AND A.PRODUCT_CODE=B.PRODUCT_CODE and B.SUB_PROD_ID='"
		 * +cardregbean.getCod_prod()+"'"); getLimitType.append(
		 * "		) and rownum=1 ");
		 * 
		 * enctrace("limitlistqry :"+ getLimitType.toString()); String
		 * limitbasedon= (String)
		 * jdbctemplate.queryForObject(getLimitType.toString(), String.class);
		 */

		// by gowtham-280819
		getLimitType.append(
				"(select PRODUCT_CODE from INSTPROD_DETAILS where  SUB_PROD_ID=? AND INST_ID=?) AND A.PRODUCT_CODE=B.PRODUCT_CODE and B.SUB_PROD_ID=?");
		getLimitType.append("		) and rownum=? ");
		enctrace("limitlistqry :" + getLimitType.toString());
		String limitbasedon = (String) jdbctemplate.queryForObject(getLimitType.toString(),
				new Object[] { cardregbean.getCod_prod(), instid, cardregbean.getCod_prod(), "1" }, String.class);
		...
		// int updatereffornewcard = cardregdao.updateordernofornewcard(instid,
		// orderfornewcard,(String)getRequest().getParameter("orderreferno"),
		// jdbctemplate);
		// int updateezauthrel =
		// cardregdao.updateordeezauthrelfornewcard(instid,
		// newhashcard.toString(),oldhcardno.toString(), jdbctemplate);

		// int
		// insertcardaccountlink=cardregdao.insertcardaccountlink(instid,cardregbean,
		// jdbctemplate);

		// int insertacctdet = cardregdao.insertacctdetails(instid, cardregbean,
		// jdbctemplate);
		// int insertcustdet = cardregdao.insertcustdetails(instid,cardregbean,
		// jdbctemplate);
		// int movetoproduct =
		// cardregdao.insertproduction(instid,cardregbean,jdbctemplate);
		...
		try {
			...
			// trace("getting count inseert insertinscustdet
			// "+insertinscustdet);
			...
			if ((updateinstcardproces > 0 || insertinsacctdet > 0 || insertinscustdet > 0 && updateezcardinfo > 0)) {
				txManager.commit(trasact.status);
				addActionMessage(
						cardregbean.getChn_msk() + " Card Registered Successfully .Waiting for Authorization ");
				...
				try {
					...
					String mcardno = commondesc.getMaskedCardbyproc(instid, cardregbean.getEncrptchn(),
							"INST_CARD_PROCESS", "C", jdbctemplate);
					if (mcardno == null) {
						mcardno = cardregbean.getEncrptchn();
					}
	...
	public String getregistrationtype(String instid, String cardtype, JdbcTemplate jdbctemplate) {
		String instype = null;
		try {
			String qrydesc = "SELECT DECODE(INTAMAPTYPE,'WEBSRV','FETCH DATA USING WEB SERVICE','CBSLINK',' FETCH DATA USING CBS LINK','SCHEMA','FETCH DATA USING OTHER SCHEMA','MAN','ENTER MANUALLY')INTAMAPTYPE  FROM PRODUCTINFO WHERE INST_ID='"
					+ instid + "' and BIN='" + cardtype.substring(0, 6) + "'";
	...
	public String registerInstCardHome() {
		...
		try {
			...
			if (branchlist == null || branchlist.isEmpty()) {
				session.setAttribute("preverr", "E");
				session.setAttribute("prevmsg", "No Branch Configured....");
				return "required_home";
			}
			...
			if (prodlist == null || prodlist.isEmpty()) {
				session.setAttribute("preverr", "E");
				session.setAttribute("prevmsg", "No Product Configured....");
				return "required_home";
			}
		 ...
		 * //return "manualentry_home"; return "instmanualmap"; }
		 */
		else {
	...
	public String manualreginstant() {
		...
		try {
			...
			if (getRequest().getParameter("cardno").trim().length() != 16) {
				session.setAttribute("preverr", "E");
				session.setAttribute("prevmsg",
						"Entered  Card NO is " + card_no + "  ,Enter Valid 16 digit card number..");
			...
			String maskedchn = padsssec.getMakedCardno(card_no);
			System.out.println("decrypted card number for encod and maintanance  " + hcardno
					+ " clear chn value/orderrefno" + hcardno);

			// validbranch =
			// commondesc.getvalidbranch(instid,hcardno.toString(),maptype,jdbctemplate);
			...
			/*
			 * int samebranchcard =
			 * cardregdao.samebranchcard(instid,hcardno.toString(),br_id,
			 * jdbctemplate);
			 * 
			 * 
			 * if( samebranchcard <=0){ //subproddesc =
			 * commondesc.getSubProductdesc(instid, subprodid, jdbctemplate);
			 * session.setAttribute("preverr", "E");
			 * session.setAttribute("prevmsg", " "+card_no+
			 * " Some other Branch Card..." ); trace("[ "+ card_no +
			 * " ]Some other Branch Card ."); return "required_home"; }
			 */
			...
			if (cardrequired == 1) {
				...
				session.setAttribute("preverr", "E");
				session.setAttribute("prevmsg",
						" " + card_no + "  card is  Already Mapped with Customer ...Waiting for Authorization...");
			...
			if (curencylist == null || curencylist.isEmpty()) {
				session.setAttribute("preverr", "E");
				session.setAttribute("prevmsg", "No Currency Configured....");
				return this.findbinsformapping();
			} else {
			...
			while (itrat.hasNext()) {
				...
				// String trimmingtime="select to_date('"+APPDATE+"',
				// 'YYYY-MM-dd') from dual ";
				// String nodaysforexpiry =
				// (String)jdbctemplate.queryForObject(trimmingtime,
				// String.class);
	...
	public void getaccountype() throws Exception {
		...
		try {
			accttype = "";// commondesc.getListOfaccounttype1(instid,
							// productcode, status, jdbctemplate);
			// trace("Getting subproduct list for the product code [
			// "+productcode+" ] ...got " + accttype );
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String newcustregWithwebservice() {
		...
		try {
			 ...
			 * if(getRequest().getParameter("cardno").trim().length() !=16){
			 * session.setAttribute("preverr", "E");
			 * session.setAttribute("prevmsg", "Entered  Card NO is "+card_no+
			 * "  ,Enter Valid 16 digit card number.." ); trace("Entered  "
			 * +card_no+"  is less than 16 digit,Kindly Check ..."); return
			 * "required_home";
			 * 
			 * }
			 * 
			 * String keyid = ""; PadssSecurity padsssec = new PadssSecurity();
			 * keyid ="1"; //commondesc.getSecurityKeyid(instid, jdbctemplate);
			 ...
			 * secList.iterator(); if(!secList.isEmpty()){
			 * while(secitr.hasNext()) { Map map = (Map) secitr.next(); String
			 * eDMK = ((String)map.get("DMK")); String eDPK =
			 * ((String)map.get("DPK"));
			 * 
			 * hcardno = padsssec.getHashedValue(card_no+instid); ecardno =
			 * padsssec.getECHN(eDMK, eDPK, card_no);
			 * 
			 * 
			 * } }
			 * 
			 ...
			 * 
			 * int cardavailable =
			 * cardregdao.cardavaialbel(instid,hcardno.toString(),maptype,
			 * jdbctemplate);
			 * 
			 * 
			 * if( cardavailable ==3){ //subproddesc =
			 * commondesc.getSubProductdesc(instid, subprodid, jdbctemplate);
			 * session.setAttribute("preverr", "E");
			 * session.setAttribute("prevmsg", " "+card_no+
			 * " Card is not found in records .." ); trace("[ "+ card_no +
			 * " ]Card is not found in records ."); return "required_home"; }
			 * 
			 * int cardrequired =
			 * cardregdao.cardstatsusformap(instid,hcardno.toString(),maptype,
			 * jdbctemplate);
			 * 
			 * trace("Got regreq: " + cardrequired ); if( cardrequired ==1 ){
			 * //subproddesc = commondesc.getSubProductdesc(instid, subprodid,
			 * jdbctemplate); session.setAttribute("preverr", "E");
			 * session.setAttribute("prevmsg", " "+card_no+
			 * "  card is  Already Mapped with Customer ...Waiting for Authorization..."
			 * ); trace("[ "+ card_no +
			 * " ]card is  Already Mapped with Customer .. .Waiting for Authorization ..."
			 * ); return "required_home"; } if( cardrequired ==2 ){
			 * //subproddesc = commondesc.getSubProductdesc(instid, subprodid,
			 * jdbctemplate); session.setAttribute("preverr", "E");
			 * session.setAttribute("prevmsg", " "+card_no+
			 * " Card is not ready for mapping .." ); trace("[ "+ card_no +
			 * " ]Card is not ready for mapping .."); return "required_home"; }
			 * 
			 * validbranch =
			 * commondesc.getvalidbranch(instid,hcardno.toString(),maptype,
			 * jdbctemplate);
			 * 
			 * 
			 * if (validbranch==null){ session.setAttribute("preverr", "E");
			 * session.setAttribute("prevmsg", " "+maskedchn+
			 * "  Entered Card Number "+card_no+
			 * "  / Cardcollect branch  is not Available ..." ); trace("[ "+
			 * card_no +" ]Card is not valid for this Branch User. ..."); return
			 * "required_home"; }
			 * 
			 */
			...
			// client.resource("http://172.16.10.67:8080/InstantServiceRegister_v.1/rest/testapi");
			/*
			 * WebResource webResource = client.resource(
			 * "http://172.16.10.40:8989/RestApi/rest/crunchifyService");
			 */
			...
			while (itrat.hasNext()) {
				...
				// getjsonvalues = this.getaccountdetailsfromcbs(instid,
				// accountno);
	...
	public String schemacustdetails() {
		...
		try {
			...
			if (validbranch == null) {
				session.setAttribute("preverr", "E");
				session.setAttribute("prevmsg", " " + maskedchn + "  Entered Card Number " + card_no
						+ "  / Cardcollect branch  is not Available ...");
					...
					try {
						...
						while (rrs.next()) {
							...
							if (acct.equals("")) {
								...
								session.setAttribute("preverr", "E");
								session.setAttribute("prevmsg",
										" " + accountno + "  Account Number is Not Available...");
								trace("[ " + accountno + " ]Account Number is Not Available...");
								return "required_home";
							}
	...
	public String newCustReg() {
		...
		try {
			...
			if (!getRequest().getParameter("regtype").trim().equals("cardbase")) {
				...
				if (validbranch == null) {
					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg", " " + orderrefno + "  Entered OrderRef Number " + orderrefno
							+ "  / Cardcollect branch  is not Available ...");
				...
				if (ordervalid < 0 || ordervalid == 0) {
					// subproddesc = commondesc.getSubProductdesc(instid,
					// subprodid, jdbctemplate);
					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg",
							" " + orderrefno + "  Kindly check,orderRefNo is  Not ready to map..");
				...
				if (cardrequired > 0) {
					...
					// subprodid, jdbctemplate);
					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg", " " + orderrefno
							+ "  orderrefno is  Already Mapped with Customer ...Waiting for Authorization...");
					trace("[ " + orderrefno
							+ " ]order is  Already Mapped with Customer .. .Waiting for Authorization ...");
					return "required_home";
				}

				ecardno = orderrefno;
				message = "" + orderrefno
						+ " Order Ref Number is not valid for this Branch User....Check with valid Order Ref Number..";
				trace("[ " + orderrefno + " ]Order Ref Number is not valid for this Branch User. ...");

			}
			// cardbase
			else {
				System.out.println("inside card base reg type");
				card_no = getRequest().getParameter("cardno").trim();
				System.out.println("card_no-->"+card_no);
				maptype = "CARDBASE";

				if (getRequest().getParameter("cardno").trim().length() != 19) {
					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg",
							"Entered  Card NO is " + card_no + "  ,Enter Valid 19 digit card number..");
					trace("Entered  " + card_no + "  is less than 19 digit,Kindly Check ...");
					return "required_home";
				...
				// keyid ="1"; //commondesc.getSecurityKeyid(instid,
				// jdbctemplate);
				...
				String maskedchn = padsssec.getMakedCardno(card_no);
				System.out.println("decrypted card number for encod and maintanance" + ecardno
						+ "clear chn value/orderrefno" + hcardno);

				validbranch = commondesc.getvalidbranchinstant(instid, ecardno.toString(), maptype, jdbctemplate);
				System.out.println("validbranch--->" + validbranch);
				if (validbranch == null) {
					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg", " " + maskedchn + "  Entered Card Number " + card_no
							+ "  / Cardcollect branch  is not Available ...");
				...
				// message=""+maskedchn+" Card is not valid for this Branch
				// User....Check with valid Card..";
				// trace("[ "+ card_no +" ]Card/order is not valid for this
				// Branch User. ...");
			...
			if (!br_id.equals("000")) {
				if (!validbranch.equalsIgnoreCase(br_id)) {
					...
					addActionError("The Card Generated branch and User loged in Branch is Different, Kindly login with same Branch");
					return "required_home";
				}
			}
			...
			if (org_chn != null || org_chn == "") {
				/*
				 * String org_chn =
				 * this.getorgchnexist(instid,accountno,jdbctemplate);
				 * if(org_chn ==null ||org_chn==""){
				 * 
				 * org_chn =
				 * this.getacountinprocess(instid,accountno,jdbctemplate);
				 * 
				 * 
				 * }
				 */ // subproddesc = commondesc.getSubProductdesc(instid,
					// subprodid, jdbctemplate);
				session.setAttribute("preverr", "E");
				session.setAttribute("prevmsg", "Account Already Exist [ " + accountno + " ] with " + org_chn
						+ " Waiting for Cuatomer Mapping Authorization..");
			...
			if (regreq > 0) {
				 ...
				 */ // subproddesc = commondesc.getSubProductdesc(instid,
					// subprodid, jdbctemplate);
				session.setAttribute("preverr", "E");
				session.setAttribute("prevmsg", "Account Already Exist [ " + accountno
						+ " ] Enter the Account No In the Maintenance to find the card..");
				trace("[ " + accountno + " ] Account Number Alredy Exist. Registeration not required");
				return "required_home";
			}

			System.out.println("before card details getting");

			// List carddet =
			// cardregdao.getcarddetails(instid,ecardno,maptype,jdbctemplate);
			List carddet = cardregdao.getcarddetails(instid, hcardno.toString(), maptype, jdbctemplate);
			trace("cardforcbs" + carddet);
			if (carddet.isEmpty()) {
				session.setAttribute("preverr", "E");
			...
			while (itrat.hasNext()) {
				...
				cardregbean.setWdl_lmt_amt(WDLAMT);
				cardregbean.setOrder_ref_no((String) map.get("ORDER_REF_NO"));
				cardregbean.setChn_msk(card_no);
				//cardregbean.setChn_msk((String) map.get("MCARD_NO"));
				//card_no
				
				cardregbean.setHascard((String) map.get("HCARD_NO"));
				cardregbean.setEncrptchn((String) map.get("ORG_CHN"));
				...
				cardregbean.setCardtype(Integer.parseInt((String) map.get("CARD_TYPE_ID").toString().trim()));
                //String responseCode="";
				cardregbean = this.getaccountdetailsfromcbs(instid, accountno, cardregbean);
				System.out.println("getjsonvalues" + getjsonvalues);
				String acctnumber=cardregbean.getAccountno();
				System.out.println("acctnumber from cbs-->"+acctnumber);
				
				
				String responce_code=cardregbean.getResponcecode();
				System.out.println("responce_code-->"+responce_code);
				 //responce_code="500";
				 System.out.println("responce_code---***>"+responce_code);
				
				  if(responce_code.equals("000") || (!responce_code.equals(null))){
					 String mobilenum=cardregbean.getPhoneno();
					 System.out.println("mobilenum-->"+mobilenum);
					 /*if((mobilenum.equals(null)) || (mobilenum.equals("")) || (mobilenum.isEmpty())){
						 System.out.println("mobilenum1111-->"+mobilenum);
				 		addActionError("Mobile Numer is Empty For Account number "+acctnumber);
						//System.out.println("message-->"+message);
						trace("responce_code--->"+responce_code+" Mobile Numer is Empty For Account number");
						System.out.println("responce_code--->"+responce_code+"--- Mobile Numer is Empty For Account number");
						return "required_home";}*/
					}  
				 
				if(responce_code.equals("300")){
					addActionError("Undefined or wrong account number... aborted");
					//System.out.println("message-->"+message);
					trace("responce_code--->"+responce_code+" Undefined or wrong account number... aborted");
					System.out.println("responce_code--->"+responce_code+"---Undefined or wrong account number... aborted");
					return "required_home";
				}
				if(responce_code.equals("401")){
					addActionError("Unauthorized access");
					//System.out.println("message-->"+message);
					 System.out.println("responce_code--->"+responce_code);
					 trace("responce_code--->"+responce_code+"---Unauthorized access");
					 return "required_home";
				}
				if(responce_code.equals("500")){
					addActionError("Account number is already linked to a card");
					//System.out.println("message-->"+message);
					 System.out.println("responce_code--->"+responce_code);
					 trace("responce_code--->"+responce_code+"---Account number is already linked to a card");
					 return "required_home";
				}
				if(responce_code.equals("55")){
					addActionError("CBS Link Down");
					//System.out.println("message-->"+message);
					 System.out.println("responce_code--->"+responce_code);
					 trace("responce_code--->"+responce_code+"---CBS Link Down");
					 return "required_home";
				}
	...
	/*
	 * public void imageAction() throws IOException{ imageId =
	 * getRequest().getParameter("imageId"); String imagetype =
	 * getRequest().getParameter("imagetype"); BufferedImage originalImage; try
	 * { originalImage = ImageIO.read(getImageFile(this.imageId, imagetype ));
	 * 
	 * ByteArrayOutputStream baos = new ByteArrayOutputStream();
	 * ImageIO.write(originalImage, "jpg", baos); baos.flush(); imageInByte =
	 * baos.toByteArray(); baos.close(); } catch (IOException e) { trace(
	...
	public void imageAction() throws IOException {
		...
		try {
			...
			com.sun.image.codec.jpeg.JPEGImageDecoder jpegDecoder = JPEGCodec
		...
		try {
			...
			try {
				...
				if (data.contains("Account is not valid,Please enter a valid account")) {
					addActionError("Entered " + accountno
							+ "Account is not Available in the CBS/Kindly Contact Head Office ...");

					trace("Could not continue the customer registeration process.." + accountno
							+ ".Account is not valid");
					String str1 = registerInstCardHome();
					return str1;
				}
	...
	/*
	 * public String getaccountdetailsfromcbs( String instid, String
	 * accountno,String ecardno ) { trace(
	 * "********Generate customer details.....*************\n"); enctrace(
	 * "********Generate customer details.....*************\n"); HttpSession
	 * session = getRequest().getSession(); String charset = "UTF-8";
	 * 
	 * try{ URLConnection connection = null; BufferedReader reader = null;
	 * StringBuffer stringBuilder= new StringBuffer(); InputStreamReader in =
	 * null; try{
	 * 
	 * //String query = String.format("acc=%s",URLEncoder.encode(acctno,
	 * charset));
	 * 
	 * 
	 * connection = new
	 * URL("http://172.18.2.55:8080/ords/azsms/cmsfetch?acc="+accountno+"").
	 ...
	 * Charset.defaultCharset()); reader = new BufferedReader(in);
	 * 
	 * String line = null; while ((line = reader.readLine()) != null) {
	 * stringBuilder.append(line ); } System.out.println(
	 * "output from the required string"+stringBuilder.toString()); String data
	 * = stringBuilder.toString(); data = data.replace('|', ',');
	 * System.out.println("after replacing the pipe symbol value we can get as"
	 * +data); String[] datas= data.split(","); JSONObject js = new
	 * JSONObject(); js.put("Name", datas[0]); js.put("Mobile Number",
	 * datas[1]); js.put("Customer ID", datas[2]); js.put("Account Currency",
	 * datas[3]); js.put("DOB", datas[4]); js.put("Address1", datas[5]);
	 * js.put("Address2", datas[6]); js.put("Address3", datas[7]);
	 * js.put("Address4", datas[8]); js.put("Address5", datas[9]);
	 * js.put("Address6", datas[10]); js.put("City", datas[11]); js.put(
	 * "Country code", datas[12]);
	 * 
	 * System.out.println("getting json values " + js.toString());
	 * 
	 * String query = String.format("acc=%s",URLEncoder.encode(accountno,
	 * charset)); connection = new
	 * URL("http://172.18.2.55:8080/ords/azsms/cmsfetch?acc="+accountno+"").
	 * openConnection(); connection.setDoOutput(true);
	 * connection.setRequestProperty("Accept-Charset", charset);
	 * connection.setRequestProperty("Content-Type",
	 ...
	 * +e.getMessage()); } finally { in.close(); } //InputStream response =
	 * connection.getInputStream(); try{ reader = new BufferedReader(new
	 * InputStreamReader(connection.getInputStream())); stringBuilder = new
	 * StringBuffer(); String line = null; while ((line = reader.readLine()) !=
	 * null) { stringBuilder.append(line ); } String data =
	 * stringBuilder.toString(); data.replace("|", ","); System.out.println(
	 * "output from the required string"+stringBuilder.toString());
	 * }catch(Exception e){ System.out.println(
	 * "Exception Happend in Reading data"+e.getMessage()); }
	 * 
	 * }catch( Exception e ){ session.setAttribute("preverr","E");
	 * session.setAttribute("prevmsg",
	 * "Exception : Could not continue the customer registeration process..." );
	 * e.printStackTrace(); trace(
	 * "Exception : while getting the customer details ... " + e.getMessage() );
	 * return this.registerInstCardHome(); }
	 */

	// String instid = comInstId();
	/*
	 * try{ //trace("Getting customer details for the customer id [ "
	 * +customerid+" ] "); List custdata = cardregdao.getaccountdetaCBS(instid,
	 * accountno, jdbctemplate2); trace("Got...[ "+custdata.size()+" ]"); if(
	 * custdata == null || custdata.isEmpty() ){ session.setAttribute("preverr",
	 * "S"); session.setAttribute("prevmsg", "" );
	 * cardregbean.setCusttype("new"); cardregbean.setKycuser("1"); trace(
	 * "New Customer...."); return "inscustomer_entry"; }else{
	 * session.setAttribute("preverr", "S"); session.setAttribute("prevmsg", ""
	 * ); trace("Existing customer...need to update...."); } trace(
	 * "Getting document type...."); List documenttypelist =
	 * commondesc.gettingDocumnettype(instid, jdbctemplate); if(
	 * documenttypelist==null || documenttypelist.isEmpty() ){
	 * session.setAttribute("preverr", "E"); session.setAttribute("prevmsg",
	 * "No document type configured...." ); trace(
	 * "No document type configured...."); return "required_home"; }
	 * 
	 * 
	 * //cardregbean.setCustid(customerid); //cardregbean.setChn(chn); Iterator
	 * itr1 = custdata.iterator(); while( itr1.hasNext() ){ Map temp =
	 * (Map)itr1.next();
	 * 
	 * 
	 * 
	 ...
	 * 
	 * 
	 * int regreq = cardregdao.checkcustomeridexist(instid,CUSTOMERID,
	 * jdbctemplate); trace("Got regreq: " + regreq ); if( regreq > 0 ){
	 * //subproddesc = commondesc.getSubProductdesc(instid, subprodid,
	 * jdbctemplate); session.setAttribute("preverr", "E");
	 * session.setAttribute("prevmsg", "[ "+ CUSTOMERID +
	 * " ] Customer Already Exist. Registeration not required" ); trace("[ "+
	 * CUSTOMERID +" ] Customer Id Alredy Exist. Registeration not required");
	 * return "required_home"; }
	 * 
	 * 
	 * 
	 ...
	 * cardregbean.setCustid(CUSTOMERID); cardregbean.setFirstname(
	 * (String)temp.get("CUST_NAME") ); //cardregbean.setCUST_NAME(
	 * (String)temp.get("CUST_NAME") );
	 * 
	 ...
	 * trace("cardforcbs"+carddet); if(carddet.isEmpty()) {
	 * session.setAttribute("preverr", "E"); session.setAttribute("prevmsg",
	 * "Could not fetch the details from the insatmcms..."); } Iterator itrat =
	 * carddet.iterator();
	 * 
	 * while(itrat.hasNext()) { Map map = (Map) itrat.next();
	 * 
	 * int WDLAMT=((BigDecimal)map.get("WDL_AMT")).intValue(); int
	 * WDLCNT=((BigDecimal)map.get("WDL_CNT")).intValue(); int
	 * PURAMT=((BigDecimal)map.get("PUR_AMT")).intValue(); int
	 * PURCNT=((BigDecimal)map.get("PUR_CNT")).intValue();
	 * 
	 * 
	 * cardregbean.setWDL_LMT_AMT(WDLAMT);
	 * cardregbean.setORDER_REF_NO((String)map.get("ORDER_REF_NO"));
	 ...
	 * cardregbean.setEncrptchn((String)map.get("CARD_NO"));
	 * 
	 * cardregbean.setWDL_LMT_CNT(WDLCNT); cardregbean.setPUR_LMT_AMT(PURAMT);
	 * cardregbean.setPUR_LMT_CNT(PURCNT);
	 * 
	 ...
	 * 
	 * 
	 * } System.out.println("custdate--->"+custdata); }catch( Exception e ){
	 * session.setAttribute("preverr","E"); session.setAttribute("prevmsg",
	 * "Exception : Could not continue the customer registeration process..." );
	 * e.printStackTrace(); trace(
	 * "Exception : while getting the customer details ... " + e.getMessage() );
	...
	/*
	 * if( actiontype.equals("EDIT")){ trace("Edit customer....."); return
	 * "inscustomer_entry"; }else{ trace("View customer.....");
	 */
	// return "inscustomer_view";

	/*
	 * public String genCustomerDetails1(String instid,String accountno,String
	 * hcardno,String jdbctemplate2 ) { trace(
	 * "********Generate customer details.....*************\n"); enctrace(
	 * "********Generate customer details.....*************\n"); HttpSession
	 * session = getRequest().getSession(); try{
	 * 
	 * 
	 * trace("Getting customer details for the customer id [ "+customerid+" ] "
	 * ); List custdata = cardregdao.getCustomerDetails(instid, customerid,
	 * jdbctemplate); trace("Got...[ "+custdata.size()+" ]"); if( custdata ==
	 * null || custdata.isEmpty() ){ session.setAttribute("preverr", "S");
	 * session.setAttribute("prevmsg", "" ); cardregbean.setCusttype("new");
	 * cardregbean.setKycuser("1"); trace("New Customer...."); return
	 * "inscustomer_entry"; }else{ session.setAttribute("preverr", "S");
	 * session.setAttribute("prevmsg", "" ); trace(
	 * "Existing customer...need to update...."); }
	 * 
	 * trace("Getting document type...."); List documenttypelist =
	 * commondesc.gettingDocumnettype(instid, jdbctemplate); if(
	 * documenttypelist==null || documenttypelist.isEmpty() ){
	 * session.setAttribute("preverr", "E"); session.setAttribute("prevmsg",
	 * "No document type configured...." ); trace(
	 * "No document type configured...."); return "required_home"; }
	 * 
	 * cardregbean.setCustid(customerid); Iterator itr = custdata.iterator();
	 * while( itr.hasNext() ){ Map temp = (Map)itr.next(); String todaydate =
	 * commondesc.getDate( "dd-MM-yyyy" ); cardregbean.setFirstname(
	 * (String)temp.get("FNAME") ); cardregbean.setLastname(
	 * (String)temp.get("MNAME") ); cardregbean.setMidname(
	 * (String)temp.get("LNAME") ); cardregbean.setFirstname(
	 * (String)temp.get("FNAME") ); cardregbean.setLastname(
	 * (String)temp.get("LNAME") ); cardregbean.setMidname(
	 * (String)temp.get("MNAME") ); cardregbean.setFahtername(
	 * (String)temp.get("FATHER_NAME") ); cardregbean.setMothername(
	 * (String)temp.get("MOTHER_NAME") ); cardregbean.setMstatus(
	 * (String)temp.get("MARITAL_STATUS") ); String maritaldesc =
	 * commondesc.getMaritalStatus( (String)temp.get("MARITAL_STATUS") );
	 * cardregbean.setMaritaldesc(maritaldesc); if(
	 * (String)temp.get("SPOUSE_NAME") != null ){ cardregbean.setSpname(
	 * (String)temp.get("SPOUSE_NAME") ); }else{ cardregbean.setSpname( "" ); }
	 * cardregbean.setGender( (String)temp.get("GENDER") ); String genderdesc =
	 * commondesc.getGenderDesc( (String)temp.get("GENDER") );
	 ...
	 * String documentname = commondesc.getDocumentName(instid,
	 * (String)temp.get("DOCUMENT_PROVIDED"), jdbctemplate);
	 * cardregbean.setDocumentdesc(documentname); cardregbean.setDocumentid(
	 * (String)temp.get("DOCUMENT_NUMBER") );
	 * 
	 ...
	 * 
	 * //cardregbean.setNationality( (String)temp.get("NATIONALITY") );
	 * cardregbean.setCountrydesc( commondesc.getNation(
	 * (String)temp.get("NATIONALITY"), jdbctemplate ) );
	 * cardregbean.setAppdate(todaydate); cardregbean.setPhotourl(
	 * (String)temp.get("PHOTO_URL") ); cardregbean.setExistingcust(true);
	 * cardregbean.setCusttype("kyc"); cardregbean.setKycuser("1");
	 * 
	 * 
	 * 
	 * } System.out.println("custdate--->"+custdata); }catch( Exception e ){
	 ...
	 * e.printStackTrace(); trace(
	 * "Exception : while getting the customer details ... " + e.getMessage() );
	 * return this.registerInstCardHome(); } if( actiontype.equals("EDIT")){
	 * trace("Edit customer....."); return "inscustomer_entry"; }else{ trace(
	 * "View customer....."); return "inscustomer_view"; } }
	 */
		...
		try {
			...
			if (inputtype.equals("cardno")) {
				...
				if (appdata == null || appdata.isEmpty()) {
					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg",
							"Could not get Application data for the card number [ " + cardno + " ] ...");
				...
				if (customerid == null) {
					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg",
							"Could not get customer id for the card number [ " + cardno + " ] ...");
			...
			cardregbean.setCustid(Integer.parseInt((custid)));
			 ...
			 * session.setAttribute("preverr", "E");
			 * session.setAttribute("prevmsg",
			 * "Could not get nationality list...." ); trace(
			 * "Could not get nationality list...."); return "required_home"; }
			...
			if (documenttypelist == null || documenttypelist.isEmpty()) {
				session.setAttribute("preverr", "E");
				session.setAttribute("prevmsg", "No document type configured....");
			...
			/*
			 * List applicationdata = cardregdao.getApplicationNumber(instid,
			 * cardno, jdbctemplate); if( applicationdata==null ||
			 * applicationdata.isEmpty() ){ session.setAttribute("preverr","E");
			 * session.setAttribute("prevmsg","No Application data found for ["
			 * +cardno+"] "); trace("No Application data found for ["+cardno+
			 * "] "); return registerInstCardHome(); }else{ Iterator apitr =
			 * applicationdata.iterator(); while( apitr.hasNext() ){ Map apmp =
			 * (Map) apitr.next(); cardregbean.setAppdate(
			 * (String)apmp.get("APP_DATE") ); cardregbean.setAppno(
			 * (String)apmp.get("APP_NO") ); } }
			 */
	...
	public String SaveCustomerReg() throws Exception
	  {
		System.out.println("Insert mrthod====================>");
		...
		String username=	comUsername();
		String usercode =	comUserCode();
		String insthash="",  check_exist="",check_exist_process="";
		int hashinsertion=-1;
		...
		cardregbean.setLastname(lastname);
		
		String custidno= getRequest().getParameter("customerid");
		System.out.println("custidno "+custidno);
		cardregbean.setCustidno(custidno);
		
		
		
		String collectionbranch= getRequest().getParameter("branchcode");
		System.out.println("collectionbranch "+collectionbranch);
		cardregbean.setCollectionbranch(collectionbranch);
		
		String encodingname= getRequest().getParameter("customername");
		System.out.println("encodingname "+encodingname);
		cardregbean.setEncodingname(encodingname);
		
		String embossingname= getRequest().getParameter("customername");
		System.out.println("embosingname "+embossingname);
		cardregbean.setEmbossingname(embossingname);
		
		
		String Accounttype= getRequest().getParameter("accounttype");
		System.out.println("accounttype-->"+Accounttype);
		cardregbean.setAccounttype(Accounttype);
		
		String Accountnumber= getRequest().getParameter("accountno");
		System.out.println("Accountnumber-->"+Accountnumber);
		cardregbean.setAccountno(Accountnumber);
		
		/*String Accounttyp= getRequest().getParameter("accounttype");
		System.out.println("Accounttyp-->"+Accounttyp);
		cardregbean.setAccounttype(Accounttyp);*/
		
		String Acctsubtype= getRequest().getParameter("accsubtype");
		System.out.println("Acctsubtype-->"+Acctsubtype);
		if(Acctsubtype.equals("SAVINGS ACCOUNT")){
			String SAVEaccttype="01";
			cardregbean.setAcctsubtype(SAVEaccttype);
		}
		else if(Acctsubtype.equals("CURRENT ACCOUNT")){
			String CURRENTacttype="02";
			cardregbean.setAcctsubtype(Acctsubtype);
		}else{cardregbean.setAcctsubtype(Acctsubtype);}
		
		
		String currency= getRequest().getParameter("cardcurr");
		System.out.println("currency "+currency);
		cardregbean.setCurrency1(currency);
		
		/*String accountid= getRequest().getParameter("acctid");
		System.out.println("accountid"+accountid);
		cardregbean.setAccountid(accountid);*/
		
		String firstname= getRequest().getParameter("customername");
		System.out.println("firstname "+firstname);
		cardregbean.setFirstname(firstname);
		
		String middlename= getRequest().getParameter("midname");
		System.out.println("middlename "+middlename);
		cardregbean.setMidname(middlename);
		...
		cardregbean.setDob(Dob);
		 ...
		 String gender = getRequest().getParameter("sex");
			System.out.println("gender "+gender);
			cardregbean.setGender(gender);	
	   
		 String maritalstatus = getRequest().getParameter("Maritaldesc");
		System.out.println("maritalstatus "+maritalstatus);
		cardregbean.setMaritaldesc(maritalstatus);
		
		String nationality = getRequest().getParameter("nationality");
		System.out.println("nationality "+nationality);
		cardregbean.setNationality(nationality);
		
		String documentprovide = getRequest().getParameter("documentprovide");
		System.out.println("documentprovide "+documentprovide);
		cardregbean.setDocumentdesc(documentprovide);
		
		String documentnumber = getRequest().getParameter("documentnumber");
		System.out.println("documentnumber "+documentnumber);
		cardregbean.setDocumentid(documentnumber);
		
		String spousename = getRequest().getParameter("spousename");
		System.out.println("spousename "+spousename);
		cardregbean.setSpname(spousename);
		

		String mothername = getRequest().getParameter("mothername");
		System.out.println("mothername "+mothername);
		cardregbean.setMothername(mothername);
		
		String fathername = getRequest().getParameter("fathername");
		System.out.println("fathername "+fathername);
		cardregbean.setFahtername(fathername);
		
		String emailid = getRequest().getParameter("emailid");
		System.out.println("emailid "+emailid);
		cardregbean.setEmail(emailid);
		
		String phonenumber = getRequest().getParameter("phonenumber");
		System.out.println("phonenumber "+phonenumber);
		cardregbean.setPhoneno(phonenumber);
		
		String p_poxbox = getRequest().getParameter("address1");
		System.out.println("p_poxbox "+p_poxbox);
		cardregbean.setP_poxbox(p_poxbox);
		
		String p_houseno = getRequest().getParameter("paddress2");
		System.out.println("p_houseno "+p_houseno);
		cardregbean.setP_houseno(p_houseno);
		
		String p_wardnumber = getRequest().getParameter("paddress3");
		System.out.println("p_wardnumber "+p_wardnumber);
		cardregbean.setP_wardnumber(p_wardnumber);
		
		String p_streetname = getRequest().getParameter("address3");
		System.out.println("p_streetname "+p_streetname);
		cardregbean.setP_streetname(p_streetname);
		
		String p_district = getRequest().getParameter("address4");
		System.out.println("p_district "+p_district);
		cardregbean.setP_district(p_district);
		
		 
			
		String p_city = getRequest().getParameter("city");
		System.out.println("city -->"+p_city);
		cardregbean.setCity(p_city);
		
		String countryCode=getRequest().getParameter("countrydesc");
		System.out.println("Country Code-->"+countryCode);
		cardregbean.setCountrydesc(countryCode);
		
		String p_phone1 = getRequest().getParameter("p_phone1");
		System.out.println("p_phone1  "+p_phone1);
		cardregbean.setP_phone1(p_phone1); 
		
		 String p_phone2 = getRequest().getParameter("p_phone2");
		System.out.println("p_phone2 "+p_phone2);
		cardregbean.setP_phone2(p_phone2); 
		
		String	CIN_NO ="";int updateref = -1;
		String Encrptchn = (String)getRequest().getParameter("Encrptchn");
		System.out.println("Encrptchn "+Encrptchn);
		
			 cardregbean.setCardno(Encrptchn);
			 cardregbean.setCustname((String)getRequest().getParameter("customername"));
			
			String branchcode= getRequest().getParameter("branchcode");
			System.out.println("branchcode "+branchcode);
			//int bracnh=Integer.parseInt(branchcode);
			 cardregbean.setBranchcode(branchcode);
			...
			/*	synchronized (this) 
				...
				try{
				  check_exist="SELECT COUNT(*) FROM CARD_PRODUCTION WHERE ACCOUNT_NO='"+cardregbean.getAccountno()+"' and CIN='"+cardregbean.getCustidno()+"'";
				  enctrace("check_exist-->"+check_exist);
				  int x = jdbctemplate.queryForInt(check_exist);
				  
				  check_exist_process="SELECT COUNT(*) FROM INST_CARD_PROCESS WHERE ACCT_NO='"+cardregbean.getAccountno()+"' and CIN='"+cardregbean.getCustidno()+"'";
				  enctrace("check_exist_inst_card_process-->"+check_exist_process);
				  int y = jdbctemplate.queryForInt(check_exist_process);
				  
				  if(x!=0){
					  addActionError("The Account Number Or Customer Id already exist in Production");
					  return "required_home";
				  }
				  if(y!=0){
					  addActionError("The Account Number Or Customer Id already exist in Process State");
					  return "required_home";
				  }
				  
				}catch(Exception e){
					trace("getting if informatin exist in production " +e.getMessage());
					return "required_home";
				}
				 
				 
		  int updateinstcardproces = cardregdao.updatefornewcardreg(instid,usercode,cardregbean,Encrptchn,jdbctemplate);   
		  int insertinsacctdet = cardregdao.insertinsacctdetails(instid, cardregbean, jdbctemplate);
		  int insertinscustdet = cardregdao.insertinscustdetails(instid,cardregbean, jdbctemplate);
		  
		  try{
		    insthash= "UPDATE INST_CARD_PROCESS_HASH SET ACCT_NO='"+cardregbean.getAccountno()+"',CIN='"+cardregbean.getCustidno()+"' WHERE ORDER_REF_NO IN (SELECT ORDER_REF_NO FROM INST_CARD_PROCESS WHERE ORG_CHN='"+Encrptchn+"')";
		    enctrace( "updatefornewcardreg : " + insthash);
		    hashinsertion = jdbctemplate.update(insthash);
		  }catch (Exception e){
				
				trace("getting execptipn updatefornewcardreghashtable " +e.getMessage());
			}
		...
		// int insertcustdet = cardregdao.insertcustdetails(instid,cardregbean, jdbctemplate);
		 //int movetoproduct = cardregdao.insertproduction(instid,cardregbean,jdbctemplate);
		  ...
		  try {
				...
				trace("getting count updation inst_card_process_hash "+hashinsertion); 
			...
			//	trace("getting count update atmcms "+insertacctdet);
				...
				//trace("getting count inseert movetoproduct "+movetoproduct); 
					...
					//txManager.commit(trasact.status);
					 if (updateinstcardproces >0 &&insertinsacctdet > 0 && insertinscustdet > 0 && hashinsertion>0 ){
					    	txManager.commit(trasact.status);
...
///START********************
	...
	public String SaveCustomerReg() throws Exception {
		...
		String encodingname= getRequest().getParameter("customername");
		String oorgchn = getRequest().getParameter("chn_msk");
		System.out.println("encodingname--name->"+encodingname);
		System.out.println("oorgchn"+oorgchn);
		//cardregbean.setEncodingname(encodingname);
		...
		String currency= getRequest().getParameter("currcode");
		System.out.println("currency"+currency);
		...
		try {
			...
			String CIN_NO = "";
			int updateref = -1;
			 String hashedcard = (String)getRequest().getParameter("hashcard");
			 System.out.println("hash card"+hashcard);
			 String cardnumber =
			 (String)getRequest().getParameter("cardnumber");
			 System.out.println("hash card"+cardnumber);
			String orgchn = (String) getRequest().getParameter("orgchn");
			System.out.println("orgchn"+orgchn);

			 cardregbean.setEncrptchn((String)getRequest().getParameter("encrptchn"));
		     cardregbean.setChn_msk((String)getRequest().getParameter("chnmask"));
		     
		  // System.out.println(cardregbean.setEncrptchn((String)getRequest().getParameter("encrptchn")));  
		   //System.out.println();  cardregbean.setChn_msk((String)getRequest().getParameter("chnmask"));
			...
			String Chn = (String) getRequest().getParameter("Chn");
			System.out.println("orgchn"+Chn);
			
			hashcard = padsssec.getHashedValue(Chn + instid);
			trace("hashcard----->" + hashcard);
			System.out.println("hashcard----->" + hashcard);
			String CDPK = padsssec.decryptDPK(CDMK, EDPK);
			// trace("CDPK-->"+CDPK);
			...
			// String WDLLMTAMT= (String)
			// getRequest().getParameter("wdl_lmt_amt");
			int WDL_LMT_AMT = 0;// Integer.parseInt(WDLLMTAMT);

			cardregbean.setWdl_lmt_amt(WDL_LMT_AMT);

			// String WDLLMTCNT= (String)
			// getRequest().getParameter("wdl_lmt_cnt");
			int WDL_LMT_CNT = 0;// Integer.parseInt(WDLLMTCNT);

			cardregbean.setWdl_lmt_cnt(WDL_LMT_CNT);

			// String PURLMTAMT= (String)
			// getRequest().getParameter("pur_lmt_amt");
			int PUR_LMT_AMT = 0;// Integer.parseInt(PURLMTAMT);

			cardregbean.setPur_lmt_amt(PUR_LMT_AMT);

			// String PURLMTCNT= (String)
			// getRequest().getParameter("pur_lmt_cnt");
			int PUR_LMT_CNT = 0;// Integer.parseInt(PURLMTCNT);
			 ...
			 * cardregbean.setCard_ccy((String)getRequest().getParameter(
			 * "cardcurr")); String currcode="";
			 * 
			 * if(cardregbean.getCard_ccy().equalsIgnoreCase("AFN")){
			 * 
			 * currcode = "971"; cardregbean.setCard_ccy(currcode); }else
			 * if(cardregbean.getCard_ccy().equalsIgnoreCase("USD")){ currcode =
			 * "840"; cardregbean.setCard_ccy(currcode); }else{
			 ...
			 * enctrace("limitlistqry :"+ getLimitType.toString()); String
			 * limitbasedon= (String)
			 * jdbctemplate.queryForObject(getLimitType.toString(),
			 * String.class);
			...
			int updateinstcardproces = cardregdao.updatefornewcardreg(instid, usercode, cardregbean, encryptedcard,
					jdbctemplate);
			int insertinsacctdet = cardregdao.insertinsacctdetails(instid, cardregbean, jdbctemplate);
			int insertinscustdet = cardregdao.insertinscustdetails(instid, cardregbean, jdbctemplate);
			...
			// int insertacctdet = cardregdao.insertacctdetails(instid,
			// cardregbean, jdbctemplate);
			// int insertcustdet =
			// cardregdao.insertcustdetails(instid,cardregbean, jdbctemplate);
			// int movetoproduct =
			// cardregdao.insertproduction(instid,cardregbean,jdbctemplate);
			...
			if (updateinstcardproces > 0 && insertinsacctdet > 0 && insertinscustdet > 0) {
				...
				try {
					...
					String mcardno = commondesc.getMaskedCardbyproc(instid, encryptedcard, "INST_CARD_PROCESS", "C",
							jdbctemplate);
					if (mcardno == null) {
						mcardno = cardregbean.getEncrptchn();
					}
					auditbean.setActmsg("Instant card " + mcardno + " Mapped to Account number from maker[ "
							+ acctnumber + " ] from " + branchcode + " ");
					auditbean.setUsercode(usercode);
...
*/
	...
	public String saveCustDetail() {
		...
		try {
			...
			// cardregbean.setOccupation(
			// getRequest().getParameter("occupation") );
			// cardregbean.setReqdocuement(
			// getRequest().getParameter("documentname") );
			// cardregbean.setDocumentid(
			// getRequest().getParameter("documentnumber") );
			...
			// cardregbean.setResaddress4(
			// getRequest().getParameter("resaddress4") );
			...
			if (custype.equals("new")) {
				...
				int newcustmove = cardregdao.insertCustomerDetailsProduction(instid, customerid, cardregbean,
						jdbctemplate);
				...
				if (newcustmove != 1) {
					...
					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg",
							"Could not move the data to production for the card [" + cardno + "] ");
				...
				int updmobileno = commondesc.updateMobileNumberToProduction(instid, cardno, cardregbean.getPhoneno(),
						jdbctemplate);
				...
				/*
				 * int updapplication = cardregdao.updateApplicationData(instid,
				 * cardno, cardregbean.getAppno(), cardregbean.getAppdate(),
				 * jdbctemplate); trace("Got : " + updapplication ); if(
				 * updapplication < 0 ){ txManager.rollback(status);
				 * session.setAttribute("preverr","E");
				 * session.setAttribute("prevmsg",
				 * "Could not continue the proces...error on update application data"
				 * ); trace(
				 * "Could not continue the proces...error on update application data...got rolled back"
				 * ); return registerInstCardHome(); }
				 */
			}
			...
			if (updatecin < 0) {
				txManager.rollback(trasact.status);
				session.setAttribute("preverr", "E");
				session.setAttribute("prevmsg", "Could not update the customer id.");
			...
			session.setAttribute("prevmsg", " Customer Registered Successfully for the card [ " + cardno
					+ " ]. Generated Customer id is  " + customerid);
			 ...
			 * "Customer Registered with the customer id " + customerid ; int
			 * mail = comutil.sendMail( instid, alertid, keymsg, jdbctemplate,
			 * session, getMailSender() ); System.out.println( "mail return__" +
			 * mail); } } catch (Exception e) { e.printStackTrace(); } finally{
			...
			session.setAttribute("preverr", "S");
			session.setAttribute("prevmsg",
					"KYC Done Successfully. Card number [ " + cardno + " ]  Customer id[ " + customerid + " ]");
	...
	public String authInstCustomerHome() {
		...
		/*
		 * if(br_id.equals("000")) { session.setAttribute("preverr", "E");
		...
		try {
			/*
			 * List branchlist = this.commondesc.generateBranchList(instid,
			 * jdbctemplate); if( branchlist== null || branchlist.isEmpty() ){
			 * session.setAttribute("preverr", "E");
			 * session.setAttribute("prevmsg", "No Branch Configured....");
			 * return "required_home"; } cardregbean.setBranchlist(branchlist);
			 */

			/*
			 * List prodlist = commondesc.getProductList( instid, jdbctemplate,
			 * session ); if( prodlist== null || prodlist.isEmpty() ){
			 * session.setAttribute("preverr", "E");
			 * session.setAttribute("prevmsg", "No Product Configured....");
			 * return "required_home"; } cardregbean.setProdlist(prodlist);
			 */
			/*
			 * trace("Getting product list...."); List prodlist =
			 * cardactdao.getinstProductList( instid, processstatus,
			 * processtype, jdbctemplate ); if( prodlist==null ||
			 * prodlist.isEmpty() ){ addActionError( "No Cards for Issue....");
			 * trace("No Cards for Issue...."); return "required_home"; }
			 * ListIterator proditr = prodlist.listIterator(); while(
			 * proditr.hasNext() ){ Map prodmp = (Map) proditr.next(); String
			 * prodcode = (String)prodmp.get("PRODUCT_CODE"); String productdesc
			 * = commondesc.getProductdesc(instid, prodcode, jdbctemplate);
			 * prodmp.put("PRODUCT_DESC", productdesc); proditr.remove();
			 * proditr.add(prodmp); } cardactbean.setProdlist(prodlist);
			 */
	...
	public String authInstCustomerList() throws Exception {
		...
		// commondesc.waitingForInstCardProcessCards(instid, card_status,
		// mkckstatus, filtercond, jdbctemplate);
		// test

		List waitingforcardpin = cardactdao.authgetCardActivationWaitingCards(instid, prodcode, card_status,
				processtype, filtercond, jdbctemplate);
	...
	public synchronized String InstconfirmCardissueorders() {
		...
		/*
		 * String ecard = getRequest().getParameter("ecard"); trace(
		 * "checking comming ecard number--->"+ecard); String mcard =
		 * getRequest().getParameter("mcard"); trace(
		 * "checking comming mcard number--->"+mcard);
		 */
		...
		String keyid = commondesc.getSecurityKeyid(instid, jdbctemplate);
		PadssSecurity padsssec = new PadssSecurity();
		String CDMK = "", CDPK = "",clearCardNumber="NA";
		...
		try {
			for (int i = 0; i < ordercount; i++) {
				clearCardNumber = padsssec.getCHN(CDPK, ecard[i]);
				...
				if (getRequest().getParameter("authorize") != null) {
					...
					if (cafflag.equals("D")) {
						 ...
						 * "UPDATE CARD_PRODUCTION SET CARD_STATUS='"
						 * +cardstatus+"', STATUS_CODE='"+switchstatus+
						 ...
						 * "' "); // }else{ // update_production.append(
						 * "AND ORG_CHN='"+ecard[i]+"' "); // //}
						 * 
						 * enctrace("update_production :"
						 * +update_production.toString()); update_product =
						 * jdbctemplate.update(update_production.toString());
						 */

						//// bygowtham-280819
						update_production.append(
								"UPDATE CARD_PRODUCTION SET CARD_STATUS=?, STATUS_CODE=?,MKCK_STATUS = ? ,  ISSUE_DATE=(SYSDATE),ACTIVE_DATE=(sysdate),MAKER_ID=?, ");
						...
						update_product = jdbctemplate.update(update_production.toString(),
								new Object[] { cardstatus, switchstatus, "p", userid, instid, ecard[i] });

						// by siva

						String hcard_no = "SELECT HCARD_NO FROM INST_CARD_PROCESS_HASH WHERE ORDER_REF_NO IN(SELECT ORDER_REF_NO FROM INST_CARD-PROCESS WHERE ORG_CHN='"
								+ ecard[i] + "') ";
						 ...
						 * updEzcard.append(",STATUS = '"+switchstatus+
						 * "' WHERE CHN = '"+hcard_no+"' AND INSTID='"
						 * +instid+"'");
						 * 
						...
						updezcardinfo = jdbctemplate.update(updEzcard.toString(),
								new Object[] { "00", switchstatus, hcard_no, instid });
						 ...
						 * "DELETE FROM INST_CARD_PROCESS WHERE INST_ID='"
						 * +instid+"' AND CARD_NO='"+ecard[i]+"' ");
						 * deletefromProcess1.append(
						 * "DELETE FROM INST_CARD_PROCESS_HASH WHERE INST_ID='"
						 * +instid+"' AND HCARD_NO='"+hcard_no+"' ");
						 */
						 ...
						 * "' "); }else{ deletefromProcess.append(
						 * "AND CARD_NO='"+Hashchn[i]+"' "); }
						 */
					...
					if (cafflag.equals("NR")) {
						...
						/*
						 * String binno=
						 * "SELECT PRODUCT_CODE FROM INST_CARD_PROCESS WHERE ORG_CHN='"
						 * +orgchn+"' "; String
						 * binnumber=(String)jdbctemplate.queryForObject(binno,
						 * String.class);
						 */

						/// by gowtham-280819
						String binno = "SELECT PRODUCT_CODE FROM INST_CARD_PROCESS WHERE ORG_CHN=? ";
						String binnumber = (String) jdbctemplate.queryForObject(binno, new Object[] { orgchn },
								String.class);
						...
						enctrace("movetoproduction _reissue: " + movetoproduction1);
						...
						crdinf_3.append(" SELECT INSTID,'" + newhcardno + "' ,'" + cardtype
								+ "' ,CUSTID,TXNGROUPID,LIMITFLG,TO_DATE('" + EXPIRYDATE + "','MM/DD/YYYY'),'"
								+ statuscode + "','" + PIN_OFFSET
								+ "' ,OLDPINOFFSET,TPINOFFSET,OLDTPINOFFSET,PINRETRYCOUNT,TPINRETRYCOUNT, PVKI, LASTTXNDATE, LASTTXNTIME,CVV, PANSEQNO FROM ");
						...
						String oldorderno = "SELECT ORDER_REF_NO FROM CARD_PRODUCTION WHERE ORG_CHN='"
								+ usedchnnNEWreissedcard + "'AND ROWNUM=1";
						enctrace("oldorderno:::::" + oldorderno + "\n" + deleteinstprocess);
						String orderforoldcar = (String) jdbctemplate.queryForObject(oldorderno, String.class);

						// int updatereffornewcard =
						// cardregdao.updateordernofornewcard(instid,
						// orderfornewcard,orderforoldcar, jdbctemplate);
						int updateezauthrel = cardregdao.updateordeezauthrelfornewcard(instid, newhcardno.toString(),
								usedhcardno.toString(), jdbctemplate);
						...
						if (updateezauthrel > 0 && ezcardinfo > 0 && production_insert > 0 && delteinstprocess > 0
								&& production_insert1 > 0 && delteinstprocess1 > 0) {
							updatecount = updatecount + 1;
					...
					if (cafflag.equals("DE")) {
						 ...
						 * if(padssenable.equals("Y")) {
						 * update_production.append("AND HCARD_NO='"+Hashchn[i]+
						 * "' AND INST_ID='"+instid+"'"); }else{
						 * update_production.append("AND CARD_NO='"+Hashchn[i]+
						 * "' AND INST_ID='"+instid+"'"); }
						 */
						/*
						 * trace("update_production :"
						 * +update_production.toString()); update_product =
						 * jdbctemplate.update(update_production.toString());
						 */
						...
						update_product = jdbctemplate.update(update_production.toString(),
								new Object[] { cardstatus, switchstatus, "P", userid, instid, ecard[i], instid });

						/*
						 * StringBuilder updEzcard = new StringBuilder();
						 * updEzcard.append("UPDATE EZCARDINFO set ");
						 * updEzcard.append(
						 * "EXPIRYDATE = (SELECT EXPIRY_DATE from INST_CARD_PROCESS where "
						 * ); updEzcard.append("HCARD_NO='"+Hashchn[i]+
						 * "' AND INSTID='"+instid+"')"); updEzcard.append(
						 * ",STATUS = '"+switchstatus+
						 * "',PANSEQNO = '00'  WHERE CHN = '"+Hashchn[i]+
						 * "' AND INSTID='"+instid+"'");
						 */
						...
						updEzcard.append(",STATUS = '" + switchstatus
								+ "',PANSEQNO = '00'  WHERE CHN = (SELECT HCARD_NO FROM CARD_PRODUCTION_HASH WHERE ORDER_REF_NO IN(SELECT ORDER_REF_NO FROM CARD_PRODUCTION WHERE ORG_CHN='"
								+ ecard[i] + "')) AND INSTID='" + instid + "'");
						...
						deletefromProcess.append("DELETE FROM INST_CARD_PROCESS WHERE INST_ID='" + instid
								+ "' AND ORG_CHN='" + ecard[i] + "' ");
						deletefromProcess1.append("DELETE FROM INST_CARD_PROCESS_HASH WHERE INST_ID='" + instid
								+ "' ORDER_REF_NO IN(SELECT ORDER_REF_NO FROM INST_CARD_PROCESS WHERE ORG_CHN='"
								+ ecard[i] + "'");
					...
					if (cafflag.equals("A")) {
						System.out
								.println("############################################################################"
										+ cafflag);
						try {
							personalcardissue = InstpersonalCardIssuence(ecard[i], padssenable, instid, userid,
							...
							/*
							 * int update =
							 * cbsTableUpdateDetails(Hashchn[i],padssenable,
							 * instid,userid,padsssec,eDMK,eDPK,cafflag,
							 * jdbctemplate); if(update == 1){ enctrace(
						...
						String custcin = commondesc.fchCustomerId(instid, padssenable, ecard[i], "INST_CARD_PROCESS",
								jdbctemplate);
						...
						String deletefromProcess = "DELETE FROM INST_CARD_PROCESS WHERE INST_ID='" + instid
								+ "' AND ORG_CHN='" + ecard[i] + "' ";
						String deletefromProcess1 = "DELETE FROM INST_CARD_PROCESS_HASH WHERE INST_ID='" + instid
								+ "' AND ORDER_REF_NO IN(SELECT ORDER_REF_NO FROM CRAD_PRODUCTION WHERE ORG_CHN='"
								+ ecard[i] + "')";

						String reissuedate = "''", reissue_count = "0", repindate = "''", repincount = "0",
								damgedate = "''", blockdate = "''", hotdate = "''", closedte = "''",
								pinretry_count = "0";
						String active_date = "''";
						...
						String movetoproduction = "INSERT INTO CARD_PRODUCTION (INST_ID,ORDER_REF_NO,BIN,MCARD_NO,ACCOUNT_NO,ACCTTYPE_ID, ACCTSUB_TYPE_ID, ACC_CCY,CIN,CARD_TYPE_ID,SUB_PROD_ID,PRODUCT_CODE,BRANCH_CODE,CARD_STATUS,CAF_REC_STATUS,STATUS_CODE,CARD_CCY,PC_FLAG,ORDER_FLAG,ORG_CHN,USED_CHN,GENERATED_DATE,"
								...
								+ cardstatus + "',CAF_REC_STATUS," + status_code
								...
								+ "',AUTO_ACCT_FLAG,OLD_PIN_OFFSET,CVV1,CVV2,ICVV,'',CARD_REF_NO FROM INST_CARD_PROCESS "
						...
						mv.append("'" + instid + "' INSTID_1, '" + ecard[i]
						...
						// -- EZCARDINFO start
						// --INSTID, CHN, CARDTYPE, CUSTID, TXNGROUPID,
						// LIMITFLG, EXPIRYDATE, STATUS, PINOFFSET,
						// OLDPINOFFSET, TPINOFFSET, OLDTPINOFFSET,
						// PINRETRYCOUNT, TPINRETRYCOUNT, PVKI, LASTTXNDATE,
						// LASTTXNTIME, PANSEQNO

						mv.append("'" + instid + "' INSTID_3, PCP.HCARD_NO CHN_3, PCP.CARD_TYPE_ID CARDTYPE_3,'"
								+ custcin + "' CUSTID_3,'01' TXNGROUPID_3, ");

						// by siva
						/*
						 * if(padssenable.equals("Y")){ mv.append("'"+instid+
						 * "' INSTID_3, PCP.HCARD_NO CHN_3, PCP.CARD_TYPE_ID CARDTYPE_3,'"
						 * +custcin+"' CUSTID_3,'01' TXNGROUPID_3, "); } else {
						 * mv.append("'"+instid+
						 * "' INSTID_3, PCP.CARD_NO CHN_3, PCP.CARD_TYPE_ID CARDTYPE_3,'"
						 * +custcin+"' CUSTID_3,'01' TXNGROUPID_3, "); }
						 */
						mv.append(
						...
						mv.append("WHERE ");
						// mv.append("WHERE (CI.ORDER_REF_NO = AI.ORDER_REF_NO
						// AND AI.ORDER_REF_NO = PCP.ORDER_REF_NO AND
						// PCP.ORDER_REF_NO = CI.ORDER_REF_NO) AND ");
						mv.append(
								"(CI.CIN = AI.CIN AND AI.CIN = PCP.CIN AND PCP.CIN = CI.CIN and pcp.acct_no=ai.ACCOUNTNO) AND ");
						mv.append(
								"(CI.INST_ID = AI.INST_ID AND AI.INST_ID = PCP.INST_ID AND PCP.INST_ID = CI.INST_ID) ");
						...
						crdinf_3.append("('" + instid + "','" + CHN_3 + "','" + CARDTYPE_3 + "','" + CUSTID_3 + "','"
								+ TXNGROUPID_3 + "','" + LIMITFLAG_3 + "',TO_DATE('" + EXPIRYDATE_3
								+ "','MM/DD/YYYY'),'" + STATUS + "','" + PINOFFSET_3 + "','" + OLDPINOFFSET_3 + "',");
						crdinf_3.append("'" + TPINOFFSET_3 + "','" + OLDTPINOFFSET_3 + "','" + PINRETRYCOUNT_3 + "','"
						...
						authrel_1.append("('" + instid + "','" + CHN_1 + "','" + ACCOUNTNO_1 + "','" + ACCOUNTTYPE_1
								+ "','" + ACCOUNTFLAG_1 + "','" + ACCOUNTPRIORITY_1 + "','" + CURRCODE_1 + "') ");
						 ...
						 * enctrace("authrel_1::::"+authrel_1.toString());
						 * 
						 * res_3 = jdbctemplate.update(crdinf_3.toString(),new
						 * Object[]{instid,CHN_3,CARDTYPE_3,CUSTID_3,
						 * TXNGROUPID_3,LIMITFLAG_3,EXPIRYDATE_3,STATUS,
						 * PINOFFSET_3,OLDPINOFFSET_3,TPINOFFSET_3,
						 * OLDTPINOFFSET_3,PINRETRYCOUNT_3,
						 * TPINRETRYCOUNT_3,PVKI_3,LASTTXNDATE_3,LASTTXNTIME_3,
						 * "0",PANSEQNO_3});
						 * 
						 * res_1 = jdbctemplate.update(authrel_1.toString(),new
						 * Object[]{instid,CHN_1,ACCOUNTNO_1,ACCOUNTTYPE_1,
						 * ACCOUNTFLAG_1,ACCOUNTPRIORITY_1,CURRCODE_1});
						 * 
						 */
						...
						if (res_1 > 0 && res_3 > 0 && production_insert > 0 && deletefromprocess > 0
								&& production_insert1 > 0 && deletefromprocess1 > 0) {
					...
					/*
					 * int update =
					 * cbsTableUpdateDetails(Hashchn[i],padssenable,instid,
					 ...
					 */
						 ...
						 * "DELETE FROM INST_CARD_PROCESS WHERE INST_ID='"
						 * +instid+"' AND HCARD_NO='"+Hashchn[i]+"' "; }else {
						 * deletefromProcess =
						 * "DELETE FROM INST_CARD_PROCESS WHERE INST_ID='"
						 * +instid+"' AND CARD_NO='"+Hashchn[i]+"' "; }
						 */

						deletefromProcess = "DELETE FROM INST_CARD_PROCESS WHERE INST_ID='" + instid + "' AND ORG_CHN='"
								+ ecard[i] + "' ";
						String deletefromProcess1 = "DELETE FROM INST_CARD_PROCESS_HASH WHERE INST_ID='" + instid
								+ "' AND ORDER_REF_NO IN(SELECT ORDER_REF_NO FROM CRAD_PRODUCTION WHERE ORG_CHN='"
								+ ecard[i] + "') ";
						...
						movetoproduction1.append(
								"INSERT INTO CARD_PRODUCTION (INST_ID,ORDER_REF_NO,BIN,HCARD_NO, ACCOUNT_NO,CIN,CARD_TYPE_ID,SUB_PROD_ID,PRODUCT_CODE,BRANCH_CODE,GENERATED_DATE,");
						movetoproduction1.append("MAKER_ID,MAKER_DATE,CHECKER_ID,CHECKER_DATE)");
						movetoproduction1.append(
								"(SELECT INST_ID,ORDER_REF_NO,BIN,HCARD_NO, ACCT_NO ,CIN,CARD_TYPE_ID,SUB_PROD_ID,PRODUCT_CODE,BRANCH_CODE,GENERATED_DATE,");
						movetoproduction1.append(
								"'" + userid + "',(SYSDATE),CHECKER_ID,CHECKER_DATE FROM INST_CARD_PROCESS_HASH ");
						movetoproduction1.append("WHERE INST_ID='" + instid
								+ "' AND ORDER_REF_NO IN(SELECT ORDER_REF_NO FROM CRAD_PRODUCTION WHERE ORG_CHN='"
								+ ecard[i] + "')");
						...
						// by siva
						/*
						 * if(padssenable.equals("Y")){ movetoproduction.append(
						 * "WHERE INST_ID='"+instid+"' AND HCARD_NO='"
						 * +Hashchn[i]+"')"); }else{ movetoproduction.append(
						...
						// PINRETRYCOUNT, TPINRETRYCOUNT, PVKI, LASTTXNDATE,
						// LASTTXNTIME, PANSEQNO
						mv.append("'" + instid + "' INSTID_3, '" + ecard[i] + "' CHN_3, PCP.CARD_TYPE_ID CARDTYPE_3,'"
								+ custcin + "' CUSTID_3,'01' TXNGROUPID_3, ");
						mv.append(
								"(SELECT CARD_FLAG FROM GLOBAL_CARDDETAILS WHERE LIMIT_TYPE = AI.ACCOUNTTYPE) LIMITFLAG_3,  ");
						mv.append("TO_CHAR(PCP.EXPIRY_DATE,'MM/DD/YYYY')  EXPIRYDATE_3,'" + status_code
						...
						// by siva
						/*
						 * if(padssenable.equals("Y")){ mv.append(
						 * "AND PCP.INST_ID='"+instid+"' AND PCP.HCARD_NO='"
						 * +Hashchn[i]+"'"); }else{ mv.append(
						 * "AND PCP.INST_ID='"+instid+"' AND PCP.CARD_NO='"
						 * +Hashchn[i]+"'"); }
						 */
						...
						enctrace("#################################");
						// enctrace("####### BULK RENUED WITH NEW card
						// ###"+Hashchn[i]);
						// trace("####### BULK RENUED WITH NEW card
						// ###"+Hashchn[i]);
						...
						StringBuilder movetoproduction1 = new StringBuilder();
						movetoproduction1.append(
								"INSERT INTO CARD_PRODUCTION_HASH (INST_ID,ORDER_REF_NO,BIN,HCARD_NO, ACCOUNT_NO,CIN,CARD_TYPE_ID,SUB_PROD_ID,PRODUCT_CODE,BRANCH_CODE,GENERATED_DATE,");
						movetoproduction1.append("MAKER_ID,MAKER_DATE,CHECKER_ID,CHECKER_DATE)");
						movetoproduction1.append(
								"(SELECT INST_ID,ORDER_REF_NO,BIN,HCARD_NO, ACCT_NO ,CIN,CARD_TYPE_ID,SUB_PROD_ID,PRODUCT_CODE,BRANCH_CODE,GENERATED_DATE,");
						movetoproduction1
								.append("'" + userid + "',(SYSDATE),CHECKER_ID,CHECKER_DATE FROM INST_CARD_PROCESS ");
						movetoproduction1.append("WHERE INST_ID='" + instid
								+ "' AND ORDER_REF_NO IN(SELECT ORDER_REF_NO FROM CARD_PRODUCTION WHERE ORG_CHN='"
								+ ecard[i] + "'))");
						 ...
						 * ACCOUNTNO_1+"','"+ACCOUNTTYPE_1+"','"+ACCOUNTFLAG_1+
						 * "','"+ACCOUNTPRIORITY_1+"','"+CURRCODE_1+"') ");
						 * enctrace("authrel_1::::"+authrel_1.toString());
						 * 
						 * 
						...
						enctrace("crdinf_3:::::" + crdinf_3.toString());
						res_3 = jdbctemplate.update(crdinf_3.toString(),
								new Object[] { instid, CHN_3, CARDTYPE_3, CUSTID_3, TXNGROUPID_3, LIMITFLAG_3,
						...
						authrel_1.append("(?,?,?,?,?,?,?) ");
						enctrace("authrel_1::::" + authrel_1.toString());
						res_1 = jdbctemplate.update(authrel_1.toString(), new Object[] { instid, CHN_1, ACCOUNTNO_1,
								ACCOUNTTYPE_1, ACCOUNTFLAG_1, ACCOUNTPRIORITY_1, CURRCODE_1 });
					...
					if (cafflag.equals("BR")) {
						 ...
						 * updEzcard.append(
						 * "EXPIRYDATE = (SELECT EXPIRY_DATE from INST_CARD_PROCESS where "
						 * ); updEzcard.append("ORG_CHN='"+ecard[i]+
						 * "' AND INSTID='"+instid+"')"); updEzcard.append(
						 * ",STATUS = '"+switchstatus+
						 * "' WHERE CHN = (SELECT HCARD_NO FROM CARD_PRODUCTION_HASH WHERE ORDER_REF_NO IN(SELECT ORDER_REF_NO FROM CARD_PRODUCTION WHERE ORG_CHN='"
						 * +ecard[i]+"'))  AND INSTID='"+instid+"'");
						 * 
						 * trace("updEzcard :"+updEzcard.toString());
						 * updezcardinfo =
						...
						updEzcard.append(
								",STATUS = ? WHERE CHN = (SELECT HCARD_NO FROM CARD_PRODUCTION_HASH WHERE ORDER_REF_NO IN(SELECT ORDER_REF_NO FROM CARD_PRODUCTION WHERE ORG_CHN=?))  AND INSTID=? ");
						trace("updEzcard :" + updEzcard.toString());
						updezcardinfo = jdbctemplate.update(updEzcard.toString(),
								new Object[] { ecard[i], instid, switchstatus, ecard[i], instid });
					...
					/*
					 * if(padssenable.equals("Y")){
					 * Fee_cardno="HCARD_NO='"+Hashchn[i]+"'"; }else{
					 ...
					 * +acctsubtype+"'"; String cnt
					 * =(String)jdbctemplate.queryForObject(checkvalidqry,String
					 * .class);
					 */
					...
					if ("0".equalsIgnoreCase(cnt)) {
						...
						// prop.getProperty("fee.glaccountno");
						// String F_TAXGLACCOUNTNO =
						// prop.getProperty("fee.taxglaccountno");
						...
						List checkfeedetails = commondesc.feeinsertactivity(instid, F_cardno, F_hcardno, F_mcardno,
								F_accountno, cafflag, F_feeid, userid, jdbctemplate);
						if (!checkfeedetails.isEmpty()) {
							...
							int checkinsertfeedet = commondesc.insertfeedetails(instid, F_cardno, F_hcardno, F_mcardno,
									F_accountno, cafflag, F_feeid, userid, mp, branchcode, jdbctemplate);
							...
							int glcheckinsertfeedet = commondesc.glinsertfeedetails(instid, F_cardno, F_hcardno,
									F_mcardno, F_GLACCOUNTNO, F_caf_rec_status, F_feeid, userid, mp, custamt,
					...
					try {
						...
						// commondesc.insertAuditTrail(instid, username,
						...
						/*
						 * System.out.println(
						 * "checking values for update product "+
						 * update_product); System.out.println(
						 * "checking values for del_process "+ del_process);
						 * System.out.println(
						 * "checking values for updezcardinfo "+ updezcardinfo);
						 * 
						 * if( del_process >0 ) {
						 * 
						 * updatecount = updatecount + 1; }else{
						 * System.out.println("Process Breaked ====> 1"); break;
						 * }
						 */
					...
					String usedchnnNEWreissedcard = "", orgchn = "", orderfornewcard = "", newhcardno = "",
							usedecardno = "", cardcon = "", newhencyptedcardno = "", cardnumber = "", PIN_OFFSET = "",
							EXPIRYDATE = "", cardtype = "";
					StringBuffer usedhcardno = new StringBuffer();
					...
					/*
					 * if(padssenable.equals("Y")){ custcin =
					 * commondesc.persfchCustomerId(instid,padssenable,
					 * Hashchn[i], "INST_CARD_PROCESS", jdbctemplate); }else{
					 * custcin =
					 * commondesc.persfchCustomerId(instid,padssenable,
					 * Hashchn[i], "INST_CARD_PROCESS", jdbctemplate); }
					 */

					// String cardcon = "";
					custcin = commondesc.persfchCustomerId(instid, padssenable, ecard[i], "INST_CARD_PROCESS",
							jdbctemplate);
					...
					int cardvailablle = commondesc.getcarddetailshcard(instid, padssenable, usedchnnNEWreissedcard,
							"INST_CARD_PROCESS", jdbctemplate);
...

					{
						...
						update_productiontab
								.append("UPDATE IFD_CARD_PRODUCTION  SET  CARD_STATUS='03', STATUS_CODE='74' ");
						update_productiontab.append(
								" WHERE INST_ID='" + instid + "' AND ORG_CHN ='" + usedchnnNEWreissedcard + "'");

						update_process
								.append("UPDATE IFD_INST_CARD_PROCESS  SET  CARD_STATUS='05',ACCT_NO='',ACCTTYPE_ID='',USED_CHN=ORG_CHN, MKCK_STATUS='P' ,REMARKS='"
										+ remarks + "',CAF_REC_STATUS='A'");
						...
						if (updateproduction > 0 && updateprocess > 0) {
							...
							update_production
									.append(" WHERE CHN in (select hcard_no from ifd_card_production where INSTID='"
											+ instid + "' AND ORG_CHN ='" + usedchnnNEWreissedcard + "')");
						...
						deletefromaccount = "DELETE FROM IFD_ACCOUNTINFO WHERE INST_ID='" + instid + "' and cin='"
								+ custcin + "'";
						deletefromcust = "DELETE FROM IFD_CUSTOMERINFO WHERE INST_ID='" + instid + "' and cin='"
								+ custcin + "'";
						...
						if (deleteifdaccount > 0) {
							...
							if (deleteifdcust > 0) {
								update_production.append("UPDATE " + tablename
										+ " SET CAF_REC_STATUS='A',ACCT_NO='',ACCTTYPE_ID='',CARD_STATUS='" + cardstatus
										+ "', STATUS_CODE='',MKCK_STATUS = 'P',CHECKER_DATE=(SYSDATE),CHECKER_ID='"
										+ userid + "', ");
								update_production.append("CIN='',REMARKS='" + remarks + "' WHERE INST_ID='" + instid
										+ "' and " + cardcon + "='" + orgchn + "'");
					...
					int deleteifdcust = -1;
					int deleteifdaccount = -1, deauth_updprocess = -1;
					...
					try {
						...
						if (deauth_updprocess > 0) {
							...
							try {
								...
								String mcardno = commondesc.getMaskedCardNoFromProd(instid, ecard[i], cond,
										jdbctemplate);
								// if(mcardno==null){mcardno=Hashchn[i];}
								...
								// commondesc.insertAuditTrail(instid, username,
								// auditbean, jdbctemplate, txManager);
								commondesc.insertAuditTrailPendingCommit(instid, userid, auditbean, jdbctemplate,
										txManager);
							} catch (Exception audite) {
								trace("Exception in auditran : " + audite.getMessage());
							}
			...
			/*
			 * if(personalcardissue >0){ //txManager.commit(transact.status);
			 * //trace("getting udpate count " + upd_process);
			 * 
			 * if(upd_process > 0){ updatecount = updatecount + 1; } else {
			 * System.out.println("Process Breaked ====> in CAF REC STATUS A 1"
			 * ); txManager.rollback(transact.status); addActionError(
			 * "Unable to continue..."); return authInstCustomerHome(); }
			 * 
			 * 
			 * //trace("Order Status Updated Successfully and Commited ");
			 * //session.setAttribute("preverr", "S");
			 * //session.setAttribute("prevmsg",ordercount+
			 * " Card Mapped Sucessfully"); }else{ trace(
			 * "Process Breaked ====> in CAF REC STATUS A 2"); addActionError(
			 * "Unable to Continue..."); txManager.rollback(transact.status);
			 * return authInstCustomerHome(); } System.out.println(
			 * "############################################################################"
			 * ); }
			 */
		...
		//PersonalCardReceiveIssueAction psa = new PersonalCardReceiveIssueAction();
		
		cardLinkWithCbs(ecard, instid, username, transact,"I",clearCardNumber);
	...
	@Async
	public void cardLinkWithCbs(final String[] order_refnum, final String instid, final String username, IfpTransObj transact, final String type, final String clearCardNumber) {
	    // to be executed in the background
		
		
		final IfpTransObj transact1 = commondesc.myTranObject("ADDCUSTOMER", txManager);
		      ...
		      try {
		        // doing some work asynchronously ...
		    	  ...
		    	  for (int i = 0; i < ordercount; i++) {
		  			...
		  			try {
		  					String url = cbsapiurl+"core/api/v1.0/account/requestInstantCard";
	...
	public synchronized int InstpersonalCardIssuence(String cardno, String padssenable, String instid, String maker_id,
		...
		/*
		 * String deletefromProcess =""; if(padssenable.equals("Y")){
		 * deletefromProcess = "DELETE FROM INST_CARD_PROCESS WHERE INST_ID='"
		 * +instid+"' AND HCARD_NO='"+cardno+"' "; }else { deletefromProcess =
		 * "DELETE FROM INST_CARD_PROCESS WHERE INST_ID='"+instid+
		 * "' AND CARD_NO='"+cardno+"' "; } trace(
		 * "checking deletion in card process" + deletefromProcess);
		 */
		...
		/*
		 * }else{ cardissuetype =
		 * commondesc.getCardIssueTypeByCard(instid,padssenable, cardno,
		 * "INSTANT", jdbctemplate); }
		 */
		...
		/*
		 * }else{ custcin = commondesc.persfchCustomerId(instid,padssenable,
		 * cardno, "INST_CARD_PROCESS", jdbctemplate); }
		 */
		...
		if (cardissuetype != null && cardissuetype.equals("$SUPLIMENT")) {
			...
			String INSTID_1 = "", CHN_1 = "", BIN = "", ACCOUNTNO_1 = "", ACCOUNTTYPE_1 = "", ACCOUNTFLAG_1 = "",
					ACCOUNTPRIORITY_1 = "", CURRCODE_1 = "";
			String INSTID_2 = "", ACCOUNTNO_2 = "", ACCOUNTTYPE_2 = "", CURRCODE_2 = "", AVAILBAL_2 = "",
					LEDGERBAL_2 = "", LIMITFLAG_2 = "1", STATUS_2 = "", TXNGROUPID_2 = "", LASTTXNDATE_2 = "",
					LASTTXNTIME_2 = "", BRANCHCODE_2 = "", PRODUCTCODE_2 = "";
			String INSTID_3 = "", CHN_3 = "", CARDTYPE_3 = "", CUSTID_3 = "", TXNGROUPID_3 = "", LIMITFLAG_3 = "1",
					EXPIRYDATE_3 = "", STATUS = "", PINOFFSET_3 = "", OLDPINOFFSET_3 = "", TPINOFFSET_3 = "",
					OLDTPINOFFSET_3 = "", PINRETRYCOUNT_3 = "", TPINRETRYCOUNT_3 = "", PVKI_3 = "", LASTTXNDATE_3 = "",
			...
			mv.append("'" + instid
					+ "' INSTID_1, PCP.ORG_CHN CHN_1,PCP.ACCT_NO ACCOUNTNO_1,PCP.BIN,AI.ACCTTYPE_ID ACCOUNTTYPE_1,PCP.CARDISSUETYPE ACCOUNTFLAG_1,'1' ACCOUNTPRIORITY_1 ,AI.ACCT_CURRENCY CURRCODE_1, ");

			/*
			 * }else{ mv.append("'"+instid+
			 * "' INSTID_1, PCP.CARD_NO CHN_1,AI.ACCOUNTNO ACCOUNTNO_1,PCP.BIN,AI.ACCTTYPE_ID ACCOUNTTYPE_1,PCP.CARDISSUETYPE ACCOUNTFLAG_1,'1' ACCOUNTPRIORITY_1 ,AI.ACCT_CURRENCY CURRCODE_1, "
			 * ); }
			 */
			/*
			 * if(padssenable.equals("Y")) { cardcon1="HCARD_NO";
			 * 
			 * } else {
			 */
			...
			mv.append("'" + instid
					+ "' INSTID_2, AI.ACCOUNTNO ACCOUNTNO_2, AI.ACCTTYPE_ID ACCOUNTTYPE_2, AI.ACCT_CURRENCY CURRCODE_2, '0' AVAILBAL_2, '0' LEDGERBAL_2,  ");
			...
			mv.append("TO_CHAR(PCP.EXPIRY_DATE,'MM/DD/YYYY')  EXPIRYDATE_3,'" + status_code
					+ "' STATUS , NVL(PCP.PIN_OFFSET,0)  PINOFFSET_3, NVL(PCP.OLD_PIN_OFFSET,0) OLDPINOFFSET_3, ");
			mv.append(
					"'0' TPINOFFSET_3,'0' OLDTPINOFFSET_3,NVL(PCP.PIN_RETRY_COUNT,0) PINRETRYCOUNT_3,'0' TPINRETRYCOUNT_3, '0' PVKI_3,  ");
			mv.append(
					"TO_CHAR(SYSDATE,'MM/DD/YYYY') LASTTXNDATE_3,TO_CHAR(SYSDATE,'HH24MISS')  LASTTXNTIME_3, '00' PANSEQNO_3, ");
			// -- EZCARDINFO end
			...
			mv.append("WHERE  ");
			// mv.append(" (CI.ORDER_REF_NO = AI.ORDER_REF_NO AND
			// AI.ORDER_REF_NO = PCP.ORDER_REF_NO AND PCP.ORDER_REF_NO =
			// CI.ORDER_REF_NO) AND"); migdata changes
			mv.append("(CI.CIN = AI.CIN AND AI.CIN = PCP.CIN AND PCP.CIN = CI.CIN) AND ");
			...
			mv.append("AND CI.INST_ID='" + instid + "' AND AI.INST_ID='" + instid + "' AND PCP.INST_ID='" + instid
					+ "'  ");
			mv.append("AND CI.CIN='" + custcin + "' AND AI.CIN='" + custcin + "' AND PCP.CIN='" + custcin + "' AND "
					+ cardcon1 + "='" + cardno + "' ");
			...
			/*
			 * mv.append("SELECT "); //--EZAUTHREL start //--INSTID,
			 * CHN,ACCOUNTNO, ACCOUNTTYPE, ACCOUNTFLAG, ACCOUNTPRIORITY,
			 * CURRCODE, if(padssenable.equals("Y")){ mv.append("'"+instid+
			 * "' INSTID_1, PCP.HCARD_NO CHN_1,AI.ACCOUNTNO ACCOUNTNO_1,AI.ACCTTYPE_ID ACCOUNTTYPE_1,PCP.PC_FLAG ACCOUNTFLAG_1,'1' ACCOUNTPRIORITY_1 ,AI.ACCT_CURRENCY CURRCODE_1, "
			 * ); }else{ mv.append("'"+instid+
			 * "' INSTID_1, PCP.CARD_NO CHN_1,AI.ACCOUNTNO ACCOUNTNO_1,AI.ACCTTYPE_ID ACCOUNTTYPE_1,PCP.PC_FLAG ACCOUNTFLAG_1,'1' ACCOUNTPRIORITY_1 ,AI.ACCT_CURRENCY CURRCODE_1, "
			 * ); }
			 * if(padssenable.equals("Y")){cardcon1="HCARD_NO";}else{cardcon1=
			 * "CARD_NO";}; //--EZAUTHREL end //--EZACCOUNTINFO start
			 * //--INSTID, ACCOUNTNO, ACCOUNTTYPE, CURRCODE, AVAILBAL,
			 * LEDGERBAL, LIMITFLAG, STATUS, TXNGROUPID, LASTTXNDATE,
			 * LASTTXNTIME, BRANCHCODE, PRODUCTCODE mv.append("'"+instid+
			 * "' INSTID_2, AI.ACCOUNTNO ACCOUNTNO_2, AI.ACCTTYPE_ID ACCOUNTTYPE_2, AI.ACCT_CURRENCY CURRCODE_2, '0' AVAILBAL_2, '0' LEDGERBAL_2,  "
			 * ); mv.append(
			 * "(SELECT ACCT_FLAG FROM GLOBAL_CARDDETAILS WHERE LIMIT_TYPE = AI.ACCOUNTTYPE) LIMITFLAG_2, '"
			 * +status_code+"' STATUS_2,  "); mv.append(
			 * "'01' TXNGROUPID_2, TO_CHAR(SYSDATE,'MM/DD/YYYY') LASTTXNDATE_2,TO_CHAR(SYSDATE,'HH24MISS')  LASTTXNTIME_2,  "
			 * ); mv.append(
			 * "PCP.BRANCH_CODE BRANCHCODE_2, AI.ACCTSUB_TYPE_ID PRODUCTCODE_2, "
			 * ); //-- EZACCOUNTINFO end //-- EZCARDINFO start //--INSTID, CHN,
			 * CARDTYPE, CUSTID, TXNGROUPID, LIMITFLG, EXPIRYDATE, STATUS,
			 * PINOFFSET, OLDPINOFFSET, TPINOFFSET, OLDTPINOFFSET,
			 * PINRETRYCOUNT, TPINRETRYCOUNT, PVKI, LASTTXNDATE, LASTTXNTIME,
			 * PANSEQNO if(padssenable.equals("Y")){ mv.append("'"+instid+
			 * "' INSTID_3, PCP.HCARD_NO CHN_3, PCP.CARD_TYPE_ID CARDTYPE_3,'"
			 * +custcin+"' CUSTID_3,'01' TXNGROUPID_3, "); }else{
			 * mv.append("'"+instid+
			 * "' INSTID_3, PCP.CARD_NO CHN_3, PCP.CARD_TYPE_ID CARDTYPE_3,'"
			 * +custcin+"' CUSTID_3,'01' TXNGROUPID_3, "); } mv.append(
			 * "(SELECT CARD_FLAG FROM GLOBAL_CARDDETAILS WHERE LIMIT_TYPE = AI.ACCOUNTTYPE) LIMITFLAG_3,  "
			 * ); mv.append(
			 ...
			 * ); mv.append(
			 * "TO_CHAR(SYSDATE,'MM/DD/YYYY') LASTTXNDATE_3,TO_CHAR(SYSDATE,'HH24MISS')  LASTTXNTIME_3, '00' PANSEQNO_3, "
			 * ); //-- EZCARDINFO end //--EZCUSTOMERINFO start //--INSTID,
			 * CUSTID, NAME, DOB, SPOUSENAME, ADDRESS1, ADDRESS2, ADDRESS3,
			 * OFFPHONE, MOBILE, EMAIL, RESPHONE mv.append("'"+instid+
			 * "' INSTID_4,'"+custcin+
			 * "' CUSTID_4, CI.FNAME NAME_4, CI.DOB  DOB_4,CI.SPOUCE_NAME SPOUSENAME_4, "
			 * ); mv.append(
			 * "CI.C_HOUSE_NO ADDRESS1_4, CI.C_STREET_NAME ADDRESS2_4,CI.C_CITY ADDRESS3_4, CI.C_PHONE1 OFFPHONE_4,CI.MOBILE MOBILE_4, "
			 * ); mv.append(
			 * "CI.E_MAIL  EMAIL_4 ,CI.C_PHONE2 RESPHONE_4, PCP.LIMIT_ID LIMIT_RECORDID_5 FROM "
			 * ); //--EZCUSTOMERINFO end mv.append(
			 * " CUSTOMERINFO CI ,ACCOUNTINFO AI ,INST_CARD_PROCESS PCP ");
			 * mv.append("WHERE  "); //modifed by senthil //uncommented
			 * //mv.append(
			 ...
			 * mv.append(
			 * "(CI.INST_ID = AI.INST_ID AND AI.INST_ID = PCP.INST_ID AND PCP.INST_ID = CI.INST_ID and pcp.acct_no=ai.ACCOUNTNO) "
			 * ); mv.append("AND CI.INST_ID='"+instid+"' AND AI.INST_ID='"
			 * +instid+"' AND PCP.INST_ID='"+instid+"'  "); //commented
			 * mv.append("AND CI.CIN='"+custcin+"' AND AI.CIN='"+custcin+
			 * "' AND PCP.CIN='"+custcin+"' AND "+cardcon1+"='"+cardno+"' ");
			 * mv.append("AND "+cardcon1+"='"+cardno+"' "); //end enctrace(
			...
			String deletefromProcess = "";
			// deletefromProcess = "DELETE FROM INST_CARD_PROCESS WHERE
			// INST_ID='"+instid+"' and "+cardcon+"='"+cardno+"'";

			/*
			 * String movetoproduction = "INSERT INTO CARD_PRODUCTION  (";
			 * movetoproduction +=
			 * "INST_ID, CARD_NO, CIN, ORDER_REF_NO, CARD_STATUS, CARD_TYPE_ID, SUB_PROD_ID, PRODUCT_CODE, BRANCH_CODE, PC_FLAG, CARD_CCY, "
			 * ; movetoproduction +=
			 ...
			 * ; movetoproduction +=
			 * "BIN, AUTH_DATE, STATUS_CODE, PIN_OFFSET, OLD_PIN_OFFSET, PIN_RETRY_CNT, AUTO_ACCT_FLAG, CVV1, CVV2, ICVV, CARD_REF_NO,HCARD_NO,MCARD_NO)"
			 * ;
			 * 
			 * movetoproduction +=
			 * " SELECT INST_ID, CARD_NO, CIN, ORDER_REF_NO, CARD_STATUS, CARD_TYPE_ID, SUB_PROD_ID, PRODUCT_CODE, BRANCH_CODE, PC_FLAG, CARD_CCY,"
			 * ; movetoproduction +=
			 * "GENERATED_DATE, EXPIRY_DATE, PRE_DATE, MAKER_ID, MAKER_DATE, CHECKER_ID, CHECKER_DATE, MKCK_STATUS, SERVICE_CODE, REG_DATE, "
			 * ; movetoproduction +=
			 * "APP_DATE, PIN_DATE, RECV_DATE, SYSDATE, ACTIVE_DATE, CAF_REC_STATUS, FEE_CODE, LIMIT_ID, PRIVILEGE_CODE, WDL_AMT, "
			 * ; movetoproduction +=
			 * "WDL_CNT, PUR_AMT, PUR_CNT, ECOM_AMT, ECOM_CNT, EMB_NAME, ENC_NAME, APP_NO, ORDER_FLAG, ORG_CHN, USED_CHN, COURIER_ID, "
			 * ; movetoproduction +=
			 * "BIN, AUTH_DATE,  STATUS_CODE, PIN_OFFSET, OLD_PIN_OFFSET, PIN_RETRY_CNT, AUTO_ACCT_FLAG, CVV1, CVV2, ICVV, CARD_REF_NO,HCARD_NO,MCARD_NO FROM INST_CARD_PROCESS WHERE INST_ID='"
			 * +instid+"' and "+cardcon+"='"+cardno+"'"; enctrace(
			 * "moveCardToProduction   "+movetoproduction);
			 */
			...
			String movetoproduction = "INSERT INTO CARD_PRODUCTION (INST_ID,ORDER_REF_NO,BIN,MCARD_NO,ACCOUNT_NO,ACCTTYPE_ID, ACCTSUB_TYPE_ID, ACC_CCY,CIN,CARD_TYPE_ID,SUB_PROD_ID,PRODUCT_CODE,BRANCH_CODE,CARD_STATUS,CAF_REC_STATUS,STATUS_CODE,CARD_CCY,PC_FLAG,ORDER_FLAG,ORG_CHN,USED_CHN,GENERATED_DATE,"
					+ "EXPIRY_DATE,APP_NO,APP_DATE,PIN_DATE,PRE_DATE,PRE_FILE,REG_DATE,RECV_DATE,ISSUE_DATE,ACTIVE_DATE,MAKER_ID,MAKER_DATE,CHECKER_ID,CHECKER_DATE,MKCK_STATUS,SERVICE_CODE,FEE_CODE,LIMIT_ID,PRIVILEGE_CODE,WDL_AMT,WDL_CNT,PUR_AMT,PUR_CNT,ECOM_AMT,"
					+ "ECOM_CNT,EMB_NAME,ENC_NAME,COURIER_ID,AUTH_DATE,ADDON_FLAG,ADDON_CNT,REISSUE_DATE,REISSUE_CNT,REPIN_DATE,REPIN_CNT,DAMAGE_DATE,BLOCK_DATE,HOT_GENDATE,CLOSE_DATE,PIN_OFFSET,PIN_RETRY_CNT,AUTO_ACCT_FLAG,OLD_PIN_OFFSET,CVV1,CVV2,ICVV,MOBILENO,CARD_REF_NO,CARDISSUETYPE,RENEWALFLAG,CARD_COLLECT_BRANCH,CARD_FLAG)"
					+ "(SELECT INST_ID,ORDER_REF_NO,BIN,MCARD_NO,ACCT_NO,ACCTTYPE_ID, ACCTSUB_TYPE_ID, ACC_CCY, CIN,CARD_TYPE_ID,SUB_PROD_ID,PRODUCT_CODE,BRANCH_CODE,'"
					+ CARDACTIVATEDCODE + "',CAF_REC_STATUS," + status_code
					+ ",CARD_CCY,PC_FLAG,ORDER_FLAG,ORG_CHN,USED_CHN,GENERATED_DATE,EXPIRY_DATE,APP_NO,APP_DATE,PIN_DATE,PRE_DATE,"
					+ "PRE_FILE,REG_DATE,RECV_DATE,(SYSDATE),SYSDATE,MAKER_ID,(SYSDATE),'" + userid
					+ "',CHECKER_DATE,'P',SERVICE_CODE,FEE_CODE,LIMIT_ID,PRIVILEGE_CODE,WDL_AMT,WDL_CNT,PUR_AMT,PUR_CNT,ECOM_AMT,"
					...
					+ "',AUTO_ACCT_FLAG,OLD_PIN_OFFSET,CVV1,CVV2,ICVV,MOBILENO,CARD_REF_NO,CARDISSUETYPE,RENEWALFLAG,CARD_COLLECT_BRANCH,'P' FROM  INST_CARD_PROCESS"
			...
			String movetoproduction1 = "INSERT INTO CARD_PRODUCTION_HASH (INST_ID,ORDER_REF_NO,BIN,HCARD_NO,ACCOUNT_NO,CIN,CARD_TYPE_ID,SUB_PROD_ID,PRODUCT_CODE,BRANCH_CODE,GENERATED_DATE,"
					...
					+ " WHERE INST_ID='" + instid
					+ "' AND ORDER_REF_NO IN(SELECT ORDER_REF_NO FROM INST_CARD_PROCESS WHERE ORG_CHN ='" + cardno
					+ "'))";
			enctrace("movetoproduction  for 2: " + movetoproduction1);

			/*
			 * String movetoproduction =
			 * "INSERT INTO CARD_PRODUCTION (INST_ID,ORDER_REF_NO,BIN,CARD_NO,HCARD_NO,MCARD_NO,ACCOUNT_NO,ACCTTYPE_ID, ACCTSUB_TYPE_ID,CIN,CARD_TYPE_ID,SUB_PROD_ID,PRODUCT_CODE,BRANCH_CODE,CARD_STATUS,CAF_REC_STATUS,STATUS_CODE,CARD_CCY,PC_FLAG,ORDER_FLAG,ORG_CHN,USED_CHN,GENERATED_DATE,"
			 * +
			 * "EXPIRY_DATE,APP_NO,APP_DATE,PIN_DATE,PRE_DATE,PRE_FILE,REG_DATE,RECV_DATE,ISSUE_DATE,ACTIVE_DATE,MAKER_ID,MAKER_DATE,CHECKER_ID,CHECKER_DATE,MKCK_STATUS,SERVICE_CODE,FEE_CODE,LIMIT_ID,PRIVILEGE_CODE,WDL_AMT,WDL_CNT,PUR_AMT,PUR_CNT,ECOM_AMT,"
			 * +
			 ...
			 * +
			 * "(SELECT INST_ID,ORDER_REF_NO,BIN,CARD_NO,HCARD_NO,MCARD_NO,ACCT_NO,ACCTTYPE_ID, ACCTSUB_TYPE_ID, CIN,CARD_TYPE_ID,SUB_PROD_ID,PRODUCT_CODE,BRANCH_CODE,'"
			 * +CARDACTIVATEDCODE+"',CAF_REC_STATUS,"+status_code+
			 * ",CARD_CCY,PC_FLAG,ORDER_FLAG,ORG_CHN,USED_CHN,GENERATED_DATE,EXPIRY_DATE,APP_NO,APP_DATE,PIN_DATE,PRE_DATE,"
			 * + "PRE_FILE,REG_DATE,RECV_DATE,(SYSDATE),SYSDATE,'"+maker_id+
			 * "',(SYSDATE),CHECKER_ID,CHECKER_DATE,'P',SERVICE_CODE,FEE_CODE,LIMIT_ID,PRIVILEGE_CODE,WDL_AMT,WDL_CNT,PUR_AMT,PUR_CNT,ECOM_AMT,"
			 * + "ECOM_CNT,EMB_NAME,ENC_NAME,COURIER_ID,AUTH_DATE,'','0',"+
			 * reissuedate+",'"+reissue_count+"',"+repindate+",'"+repincount+
			 * "',"+damgedate+","+blockdate+","+hotdate+","+closedte+
			 * ",PIN_OFFSET,'"+pinretry_count+
			 * "',AUTO_ACCT_FLAG,OLD_PIN_OFFSET,CVV1,CVV2,ICVV,MOBILENO,CARD_REF_NO,CARDISSUETYPE FROM INST_CARD_PROCESS "
			...
			cinf_4.append("'" + ADDRESS1_4 + "','" + ADDRESS2_4 + "','" + ADDRESS3_4 + "','" + OFFPHONE_4 + "','"
			...
			crdinf_3.append("VALUES ");
			/*
			 * crdinf_3.append("('"+instid+"','"+CHN_3+"','"+CARDTYPE_3+"','"+
			 * CUSTID_3+"','"+TXNGROUPID_3+"','"+LIMITFLAG_3+"',TO_DATE('"+
			 * EXPIRYDATE_3+"','MM/DD/YYYY'),'"+STATUS+"','"+PINOFFSET_3+"','"+
			 * OLDPINOFFSET_3+"',");
			 */
			crdinf_3.append("('" + instid
					+ "',(select hcard_no from INST_CARD_PROCESS_HASH where ORDER_REF_NO IN(SELECT ORDER_REF_NO FROM INST_CARD_PROCESS WHERE ORG_CHN='"
					+ CHN_3 + "')),'" + CARDTYPE_3 + "','" + CUSTID_3 + "','" + TXNGROUPID_3 + "','" + LIMITFLAG_3
					+ "',TO_DATE('" + EXPIRYDATE_3 + "','MM/DD/YYYY'),'" + STATUS + "','" + PINOFFSET_3 + "','"
					+ OLDPINOFFSET_3 + "',");
			crdinf_3.append("'" + TPINOFFSET_3 + "','" + OLDTPINOFFSET_3 + "','" + PINRETRYCOUNT_3 + "','"
					+ TPINRETRYCOUNT_3 + "','" + PVKI_3 + "',TO_DATE('" + LASTTXNDATE_3 + "','MM/DD/YYYY'),'"
					+ LASTTXNTIME_3 + "'  ,'0' ,");
			crdinf_3.append("'" + PANSEQNO_3 + "' )");
			...
			/*
			 * ezac_2.append("('"+instid+"','"+ACCOUNTNO_2+"','"+ACCOUNTTYPE_2+
			 * "','"+CURRCODE_2+"','"+AVAILBAL_2+"','"+LEDGERBAL_2+"','"+
			 * LIMITFLAG_2+"','"+STATUS_2+"',");
			 */
			ezac_2.append("('" + instid + "','" + ACCOUNTNO_2 + "','" + ACCOUNTTYPE_2 + "','" + CURRCODE_2 + "','"
					+ AVAILBAL_2 + "','" + LEDGERBAL_2 + "','" + LIMITFLAG_2 + "','" + STATUS_2 + "',");
			ezac_2.append("'" + TXNGROUPID_2 + "',TO_DATE('" + LASTTXNDATE_2 + "','MM/DD/YYYY'),'" + LASTTXNTIME_2
			...
			authrel_1.append("VALUES ");
			/*
			 * authrel_1.append("('"+instid+"','"+CHN_1+"','"+ACCOUNTNO_1+"','"+
			 * ACCOUNTTYPE_1+"','"+ACCOUNTFLAG_1+"','"+ACCOUNTPRIORITY_1+"','"+
			 * CURRCODE_1+"','"+ACCOUNTFLAG_1+"') ");
			 */
			authrel_1.append("('" + instid
					+ "',(select hcard_no from INST_CARD_PROCESS_HASH where ORDER_REF_NO IN(SELECT ORDER_REF_NO FROM INST_CARD_PROCESS WHERE ORG_CHN='"
					+ CHN_3 + "')),'" + ACCOUNTNO_1 + "','" + ACCOUNTTYPE_1 + "','" + ACCOUNTFLAG_1 + "','"
					+ ACCOUNTPRIORITY_1 + "','" + CURRCODE_1 + "','" + ACCOUNTFLAG_1 + "') ");
			enctrace("authrel_1::::" + authrel_1.toString());
			...
			while (accitr.hasNext()) {
				...
				String getres_5Query = persisspro.getres_5Query(instid, LIMITTYPE_5, LIMITID_5, TXNCODE_5, CURRCODE_5,
						AMOUNT_5, COUNT_5, WAMOUNT_5, WCOUNT_5, MAMOUNT_5, MCOUNT_5, YAMOUNT_5, YCOUNT_5, LIMITDATE_5);
			...
			StringBuilder update_production = new StringBuilder();
			update_production.append("UPDATE CARD_PRODUCTION SET CARD_STATUS='" + cardstatus + "', STATUS_CODE='"
					+ switchstatus + "',MKCK_STATUS = 'P' ,  ISSUE_DATE=(SYSDATE),ACTIVE_DATE=(sysdate),MAKER_ID='"
					+ userid + "', ");
			update_production
					.append("MAKER_DATE=(sysdate) WHERE INST_ID='" + instid + "' and ORG_CHN='" + cardno + "'");

			enctrace("update query for instcard processs " + update_production);

			StringBuilder deleteifdinstproc = new StringBuilder();
			StringBuilder deleteifdinstproc1 = new StringBuilder();
			deleteifdinstproc1.append("DELETE FROM INST_CARD_PROCESS_HASH WHERE INST_ID='" + instid
					+ "' AND ORDER_REF_NO =(SELECT ORDER_REF_NO FROM INST_CARD_PROCESS WHERE ORG_CHN='" + cardno
					+ "') ");
			deleteifdinstproc.append(
					"DELETE FROM INST_CARD_PROCESS WHERE INST_ID='" + instid + "' AND ORG_CHN='" + cardno + "' ");
			trace("checking delete query--->" + deleteifdinstproc);
			trace("checking delete query--->" + deleteifdinstproc1);

			/*
			 * if(padssenable.equals("Y")){ deleteifdinstproc.append(
			 * "AND HCARD_NO='"+cardno+"' "); }else{ deleteifdinstproc.append(
			 * "AND ORG_CHN='"+cardno+"' "); }
			 */
			...
			trace("result :::::::::::" + res_1 + res_2 + res_3 + res_4 + res_5 + upd_process + production_insert
					+ production_insert1 + del_process + del_process1);

			if (res_1 > 0 && res_2 > 0 && res_3 > 0 && res_4 > 0 && res_5 > 0 && upd_process > 0
					&& production_insert > 0 && production_insert1 > 0 && del_process > 0 && del_process1 > 0) {
				trace("PROCESS COMPLETED");
				issue_status = 1;
			} else {
				issue_status = -1;
	...
	/*
	 * public String getorgchnexist(String instid, String accountno,
	 * JdbcTemplate jdbctemplate ) { String org_chn=null,processinst =
	 * null,productionavilable = null,secondaryaccount = null; try{ org_chn =
	 * "SELECT CHN FROM EZAUTHREL WHERE INSTID='"+instid+"' AND ACCOUNTNO='"
	 * +accountno+"'"; String hascard =
	 * (String)jdbctemplate.queryForObject(org_chn, String.class);
	 * enctrace("originaclchn222222222" + hascard ); processinst =
	 * "SELECT ORG_CHN FROM CARD_PRODUCTION WHERE INST_ID='"+instid+
	 * "' AND HCARD_NO='"+hascard+"'";
	 * 
	 * secondaryaccount = (String)jdbctemplate.queryForObject(processinst,
	 * String.class); enctrace("originaclchn222222222" + hascard );
	 * if(hascard==null||hascard.equals("")){ return secondaryaccount ; }
	 * }catch(EmptyResultDataAccessException e){} //return org_chn; return
	 * secondaryaccount + " As Secondary Account .." ; } public String
	 * getacountinprocess(String instid, String accountno, JdbcTemplate
	 * jdbctemplate ) { String org_chn=null,processinst =
	 * null,productionavilable = null,secondaryaccount = null; try{ String
	 * originaclchn = "SELECT ORG_CHN FROM INST_CARD_PROCESS WHERE INST_ID='"
	 * +instid+"' AND ACCT_NO='"+accountno+"'"; processinst =
	 * (String)jdbctemplate.queryForObject(originaclchn, String.class);
	 * System.out.println("processinst" + processinst );
	 * 
	 * 
	 * }catch(EmptyResultDataAccessException e){} //return org_chn; return
	 * processinst + "Wating for Customer mapping authorization.." ; }
	 */
	...
	public String findinprocess(String instid, String accountno, JdbcTemplate jdbctemplate) {
		...
		try {
			...
			/*
			 * String originaclchn =
			 * "SELECT ORG_CHN FROM INST_CARD_PROCESS WHERE INST_ID='"+instid+
			 * "' AND ACCT_NO='"+accountno+"'"; processinst =
			 * (String)jdbctemplate.queryForObject(originaclchn, String.class);
			 */

			/// BY GOWTHAM-280819
			String originaclchn = "SELECT ORG_CHN FROM INST_CARD_PROCESS WHERE INST_ID=? AND" + " ACCT_NO=? ";
			processinst = (String) jdbctemplate.queryForObject(originaclchn, new Object[] { instid, accountno },
					String.class);
	...
	/*
	 * public static void main(String[] args){ try{ Client client =
	 * Client.create();
	 * 
	 * WebResource webResource = client.resource(
	 * "http://172.16.10.175:8080/InstantServiceRegister/rest/checkFunction");
	 * WebResource webResource =
	 * client.resource("http://172.16.10.40:8989/RestApi/rest/crunchifyService")
	 * ;
	 * 
	 * JSONObject data=new JSONObject(); data.put("ACCOUNTNO", "8189968318");
	 * 
	 * // data.put("INSTID", "PMT"); System.out.println("income");
	 * 
	 * ClientResponse response = (ClientResponse)
	 * webResource.type("application/json") .post(ClientResponse.class,
	 * data.toString()); System.out.println(response.getStatus()); if
	 * (response.getStatus() != 200) { throw new RuntimeException(
	 * "Failed : HTTP error code : " + response.getStatus()); }
	 * System.out.println("Output from Server .... \n"); String output =
	 * (String) response.getEntity(String.class); System.out.println(output);
	 * 
	 * } catch (Exception e) {
	 * 
	...
	public List GetOrderNumber(String instid, JdbcTemplate jdbctemplate) throws Exception {
		...
		/*
		 * String qry =
		 * "SELECT CIN_NO, ORDER_REFNO FROM SEQUENCE_MASTER WHERE INST_ID='"
		 * +instid+"' "; List listoforder = jdbctemplate.queryForList(qry);
		 */
		...
		if (getRequest().getParameter("cardno") != "") {
			...
			if (otptype.equalsIgnoreCase("RO")) {
				...
				if (waitingforotp.isEmpty()) {
					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg", " No records found for RE-PIN Generation");
					return "pinotphome";
				}

			} else if (otptype.equalsIgnoreCase("NO")) {
				System.out.println("income forcard NO");
				waitingforotp = cardregdao.otpcardshcardnolist(instid, ecardno.toString(), newhashcard.toString(),
						jdbctemplate);
				trace("waitingNO" + waitingforotp);
				if (waitingforotp.isEmpty()) {
					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg", " No records found PIN Generation");
					return "pinotphome";
				}
			}
			...
			if (otptype.equalsIgnoreCase("RO")) {
				...
				if (waitingforotp.isEmpty()) {
					session.setAttribute("preverr", "E");
					session.setAttribute("prevmsg", " No records found for PIN Generation");
					return "pinotphome";
				}
	...
	public String otpcardslistrepin() throws Exception {
		HttpSession session = getRequest().getSession();
		String instid = comInstId();
		System.out.println("instid-->" + instid);
		String branchid = comBranchId();
	...
	public synchronized String confrimotpchangestatus() throws Exception {
		System.out.println("NEW CARD METHOD");
		trace("NEW CARD METHOD");
		trace("*****confrimotpchangestatus*****");
		enctrace("*****confrimotpchangestatus*****");
		HttpSession session = getRequest().getSession();
		IfpTransObj transact = commondesc.myTranObject("AUTHCUST", txManager);
		String instid = comInstId();
		// String newhashcard="";
		...
		for (int i = 0; i < cardcardlength; i++) {
			String cardonebyon = orgchn[i];
				...
				/*
				 * String query=
				 * "SELECT CARD_NO,MOBILENO,BIN,CARD_TYPE_ID,CIN,MCARD_NO,CAF_REC_STATUS FROM CARD_PRODUCTION WHERE INST_ID='"
				 * +instid+"'AND HCARD_NO='"+cardonebyon+"' ";
				...
				if (!result.isEmpty()) {
					...
					if (padssenable.equals("Y")) {
						...
						// newhashcard =
						// padsssec.getHashedValue(getRequest().getParameter("dcardno")+instid);
					...
					if( mobilno == null  || mobilno.equalsIgnoreCase("") || mobilno=="NULL")
					{
						addActionError("Mobile number Empty/Null for card number "+mcardno);
						return "pinotphome";
						
					}
					// String response=hsm.OTPGeneration( instid, ORG_CHN,
					// cardonebyon, mobilno, jdbctemplate );
					...
					String pinoffset = hsm.printRacalPinOtp(instid, dcardno, mcardno, cin, card_type, bin,
							"CARD_PRODUCTION", in, out, jdbctemplate, hsmobj, CHN);
					// String pinoffset = hsm.printRacalPin(instid,dcardno,
					// mcardno, cin, card_type, bin, "CARD_PRODUCTION", in, out,
					// jdbctemplate, hsmobj,CHN);
					//String pinoffset = "";
					...
					smslog("PIN SMS MOBILE NUMBER "+mobilno);
					String message = "Dear Value Customer, Welcome to Rokel Commercial Bank Ltd, " + clearpin + " is the PIN for your card "
							+ mcardno + ". Please do not share with anyone.";
					smslog("PIN SMS MESSAGE ---> "+message);
					...
					try {
						 OtpSmsCall otp=new OtpSmsCall();
					      
						String smsrespcode = "",smsresp="";
						// String smsresps = new JSONObject();
						 
						System.out.println("Before Req");
						smsrespcode =otp.uploadToServer(mobilno,message);
						System.out.println("After Req : " + smsrespcode);
 

						if (smsrespcode.contains("pending")) {
							smsresp = "Message is accepted.";
							trace("Successfully Send");
						} else if (smsrespcode.equals("1")) {
							smsresp = "Message is in pending status.";
							trace("Message is in pending status.");
						} 
						else {
							smsresp = "Bad response from sms vendor";
							trace("Bad response from sms vendor");
						}

						// String smsresp="00~SUCCESS";

						// *************************
						if (smsrespcode.contains("pending") || smsrespcode.contains("1") || smsrespcode.contains("3")) {
							String updateqry = "", updqry = "";
							if ("A".equals(cafstatus)) {
								updateqry = "UPDATE CARD_PRODUCTION SET OTPSTATUS='S',PIN_OFFSET='" + pinoff
										+ "',PIN_DATE=sysdate,CARD_STATUS='05',STATUS_CODE='53',MAKER_DATE=sysdate,MAKER_ID='"
										+ userid + "' WHERE INST_ID='" + instid + "' AND ORG_CHN='" + CHN + "'";
								updqry = "UPDATE EZCARDINFO SET STATUS='53',PINOFFSET='" + pinoff
										+ "',OTPGEN_DATE=SYSDATE WHERE INSTID='" + instid + "' AND CHN='" + hcardno
										+ "'";
							} else if ("R".equals(cafstatus)) {
								updateqry = "UPDATE EZCARDINFO SET STATUS='53',PINOFFSET='" + pinoff
										+ "',PINRETRYCOUNT='0',OTP_INTVL='0',OTPGEN_DATE=SYSDATE WHERE INSTID='"
										+ instid + "' AND CHN='" + hcardno + "'";
								updqry = "UPDATE CARD_PRODUCTION SET OTPSTATUS='S',PIN_OFFSET='" + pinoff
										+ "',STATUS_CODE='53',CARD_STATUS='05',PIN_DATE=sysdate,CAF_REC_STATUS='A' WHERE INST_ID='"
										+ instid + "' AND ORG_CHN='" + CHN + "'";
							} else {
								updateqry = "UPDATE CARD_PRODUCTION SET OTPSTATUS='S',PIN_OFFSET='" + pinoff
										+ "',PIN_DATE=sysdate,CARD_STATUS='05',STATUS_CODE='50',MAKER_DATE=sysdate,MAKER_ID='"
										+ userid + "' WHERE INST_ID='" + instid + "' AND ORG_CHN='" + CHN + "'";
								updqry = "UPDATE EZCARDINFO SET STATUS='50',PINOFFSET='" + pinoff
										+ "',OTPGEN_DATE=SYSDATE WHERE INSTID='" + instid + "' AND CHN='" + hcardno
										+ "'";
							}
							trace("updqry-->" + updqry);
							...
							if (cardcardlength == 1)
							{
								addActionMessage("OTP sent successfully to the customer for the card [ " + mcardno
										+ " ]  ");
							}
						...
						session.setAttribute("preverr", "E");
						session.setAttribute("prevmsg", "Exception : Could not generate PIN...");
						e.printStackTrace();
						failure++;
						in.close();
						out.close();
				...
				in.close();
				out.close();
				addActionError("Exception : while generating the PIN ... ");
				trace("Exception : while generating the PIN ... " + e.getMessage());
			}
		finally {
			//in.close();
			//out.close();
			txManager.commit(transact.status);
			addActionMessage("Total Count [ " + smssentcount + " ]  Success Count is."
					+ " [  "+successcount+" ]  ..  Failure count is .. [  "+failure+ " ]. "
					+ " ]");
		}
	...
	public synchronized String confrimotpchangestatusforREPIN() throws Exception {
		System.out.println("REPIN METHOD");
		trace("REPIN METHOD");
		trace("*****confrimotpchangestatus*****");
		enctrace("*****confrimotpchangestatus*****");
		HttpSession session = getRequest().getSession();
		...
		for (int i = 0; i < cardcardlength; i++) {
			String cardonebyon = orgchn[i];
				...
				if (!result.isEmpty()) {
					...
					String pinoffset = hsm.printRacalPin(instid, dcardno, mcardno, cin, card_type, bin,
							"CARD_PRODUCTION", in, out, jdbctemplate, hsmobj, CHN);
					// String pinoffset = hsm.printRacalPin(instid,dcardno,
					// mcardno, cin, card_type, bin, "CARD_PRODUCTION", in, out,
					// jdbctemplate, hsmobj,CHN);
					...
					smslog("PIN SMS MOBILE NUMBER "+mobilno);
					String message = "Dear Value Customer, Welcome to SIB Liberia Ltd, " + clearpin + " is the PIN for your card "
							+ mcardno + ". Please do not share with anyone. www.sibll.com";
					smslog("PIN SMS MESSAGE ---> "+message);
					...
					try {
						...
						System.out.println("Array Size : " + datas.length);

						String Response = datas[0];
						String Mobile_Number = datas[1];
						String MessageId = datas[2];

						// System.out.println("getting json values " +
						// js.toString());
*/
						if (smsrespcode.contains("0")) {
							smsresp = "Message is accepted.";
							trace("Successfully Send");
						} else if (smsrespcode.equals("1")) {
							smsresp = "Message is in pending status.";
							trace("Message is in pending status.");
						} else if (smsrespcode.equals("2")) {
							smsresp = "Message is undeliverable.";
							trace("Message is undeliverable.");
						} else if (smsrespcode.equals("3")) {
							smsresp = "Message is delivered.";
							trace("Message is delivered.");
						} else if (smsrespcode.equals("4")) {
							smsresp = "Message is expired.";
							trace("Message is expired.");
						}
						else if (smsrespcode.equals("5")) {
							smsresp = "Message is rejected.";
							trace("Message is rejected.");
						}
						else {
							smsresp = "Insufficient Credit Reseller";
							trace("Insufficient Credit Reseller");
						}

						// String smsresp="00~SUCCESS";

						// *************************
						if (smsrespcode.contains("0") || smsrespcode.contains("1") || smsrespcode.contains("3")) {
							String updateqry = "", updqry = "";
							if ("A".equals(cafstatus)) {
								updateqry = "UPDATE CARD_PRODUCTION SET OTPSTATUS='S',PIN_OFFSET='" + pinoff
										+ "',PIN_DATE=sysdate,CARD_STATUS='05',STATUS_CODE='50',MAKER_DATE=sysdate,MAKER_ID='"
								...
								updateqry = "UPDATE EZCARDINFO SET STATUS='50',PINOFFSET='" + pinoff
										+ "',PINRETRYCOUNT='0',OTP_INTVL='0',OTPGEN_DATE=SYSDATE WHERE INSTID='"
										+ instid + "' AND CHN='" + hcardno + "'";
								updqry = "UPDATE CARD_PRODUCTION SET OTPSTATUS='S',PIN_OFFSET='" + pinoff
										+ "',STATUS_CODE='50',CARD_STATUS='05',PIN_DATE=sysdate,CAF_REC_STATUS='A' WHERE INST_ID='"
										+ instid + "' AND ORG_CHN='" + CHN + "'";
							} else {
								updateqry = "UPDATE CARD_PRODUCTION SET OTPSTATUS='S',PIN_OFFSET='" + pinoff
										+ "',PIN_DATE=sysdate,CARD_STATUS='05',STATUS_CODE='50',MAKER_DATE=sysdate,MAKER_ID='"
			...
			//out.close();
			txManager.commit(transact.status);
			addActionMessage("OTP sent successfully to the customer for .. Total Count [ " + smssentcount + " ]  Success Count is."
					+ " [  "+successcount+" ]  ..  Failure count is .. [  "+failure+ " ]. "
					+ " ] ... card is successfully activated ");
		}

		
		return "pinotphome1";
		
		///commented on 03022021
	...
	public InstCardRegisterProcessBeans getaccountdetailsfromcbs(String instid, String accountno,
			InstCardRegisterProcessBeans cardregbean) {
		trace("********Generate customer details.....*************\n");
		enctrace("********Generate customer details.....*************\n");
		HttpSession session = getRequest().getSession();
		String charset = "UTF-8";
		...
		try {
			...
			URLClientRegistration cusreg = new URLClientRegistration();

			try {

				obj = cusreg.uploadToServer(accountno);
				//obj = cusreg.example(accountno);
				if(obj == null){
					addActionError("CBS Card Link API Down");
					//cardregbean.setResponcecode(jObj.getString("responseCode"));
					cardregbean.setResponcecode("55");
					System.out.println("resp_code-->"+obj);
					trace("CBS Card Link API Down");
					return cardregbean;
				}
				trace("returning jason object--->" + obj);

		 		String resp_code=obj.getString("responseCode");
				String message=obj.getString("message");
				cardregbean.setResponcecode(resp_code);
				...
				if(resp_code.equals("000")){
					...
					System.out.println("message-->"+message);
		 		    ...
		 		    System.out.println("arra ******11" + jObj.getString("lastName"));
		 		   System.out.println("arra ******11" + jObj.getString("accountType"));

			 		cardregbean.setLastname(jObj.getString("lastName"));
					//cardregbean.setAcctsubtype(jObj.getString("accountDescription"));
					cardregbean.setGender(jObj.getString("gender"));
					cardregbean.setPaddress3(jObj.getString("address3"));
					cardregbean.setPaddress2(jObj.getString("address2"));
					cardregbean.setCity(jObj.getString("city"));
					cardregbean.setPhoneno(jObj.getString("mobileNumber"));
					cardregbean.setPaddress1(jObj.getString("address1"));
					if(jObj.getString("accountType").toString().equals("02")){
						cardregbean.setAccttype("20");
					}else if(jObj.getString("accountType").toString().equals("01")){
						cardregbean.setAccttype("10");
					}
					
					//cardregbean.setAccttype(jObj.getString("accountType"));
					cardregbean.setDob(jObj.getString("dateOfBirth"));
					cardregbean.setCustidno(jObj.getString("customerNumber"));
					//cardregbean.setCustid(jObj.getString("customerNumber"));
					cardregbean.setAccountno(jObj.getString("fullBban"));
					cardregbean.setFirstname(jObj.getString("accountDescription"));
					cardregbean.setCountrydesc(jObj.getString("countryCode"));
					cardregbean.setCard_ccy(jObj.getString("currency"));
					cardregbean.setMidname(jObj.getString("middleName"));
					cardregbean.setMaritaldesc(jObj.getString("maritalStatus"));
					cardregbean.setBranchcode(jObj.getString("accountBranch"));
					cardregbean.setCustname(jObj.getString("accountDescription"));
					//cardregbean.setResponcecode(jObj.getString("responseCode"));
					
					 String accountnumber=jObj.getString("fullBban");
					 System.out.println("accountnumber----///////---> "+accountnumber);
				...
				else if (resp_code.equals("500")){
					addActionError("wrong account");
					//cardregbean.setResponcecode(jObj.getString("responseCode"));
					//cardregbean.setResponcecode(getString(resp_code));
					System.out.println("resp_code-->"+resp_code);
					System.out.println("message-->"+message);
					return cardregbean;
				}else if(resp_code.equals("300")){
					//cardregbean.setResponcecode(getString(resp_code));
					System.out.println("resp_code-->"+resp_code);
					System.out.println("message-->"+message);
					return cardregbean;}
				
				else if(resp_code.equals("401")){
					//cardregbean.setResponcecode(getString(resp_code));
					System.out.println("resp_code-->"+resp_code);
					System.out.println("message-->"+message);
					return cardregbean;
