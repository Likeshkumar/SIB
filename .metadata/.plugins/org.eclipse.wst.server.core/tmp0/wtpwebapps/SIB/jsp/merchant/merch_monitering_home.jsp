			...
			<tr> 
			 ...
			 <% String title = "Click here to view detail"; %>
				...
				<td class="jOrgChartbox"  >  
					...
					<s:elseif test="%{MERACHANTISSUE==2 || MERACHANTISSUE==3 }">
						<% bgstyle="warning"; 
						imagename = "images/merchant/warning_merchant.png";
						%> 
					 </s:elseif>
					<s:elseif test="%{MERACHANTISSUE==5}">
						<% bgstyle="warning"; title = "TERMINAL PROFILE AND TERMINAL STATUS COUNT IS MISMATCH";
						imagename = "images/merchant/warning_merchant.png";
						%> 
					</s:elseif> 
				<s:else>
					<% bgstyle="active"; imagename = "images/merchant/active_merchant.png";%>
				</s:else> 
					 					 ...
					 					 <a  class="ajax" style="cursor:pointer" alt="Merchant" title="<%=title%>" rel="tooltip" link="showTreeViewMerchMonitor.do?MERCHANTID=<s:property value="MERCH_ID"/>">
					 					 	<div <%=bgstyle%>"  > 
					 					 	<img src="<%=imagename%>" width="60"/>
					 						<br/>	 <s:property value="MERCH_NAME"/>
					 						 </div> 
					 					</a>
					 						
					 					 <%-- onclick="showDetailedTerminal('<s:property value="MERCH_ID"/>')" --%>
