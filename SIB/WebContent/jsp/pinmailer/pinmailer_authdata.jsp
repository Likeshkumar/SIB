...
function getReason(){
	var reasonfail = prompt("Reason for Reject ?");
	//alert( reasonfail  )
	if( reasonfail == null ){
		alert ( "false" );
		return false;
	}
	document.getElementById("reason").value=reasonfail;  
...
<s:form action="pinMailerAuthorizepinMailerConfigurationAction"  name="pinmailerdataaddform" namespace="/" autocomplete="off">
	<s:if test="%{pinmaildata_status =='Y'}">
		...
		<s:iterator  value="pinmailsdata"> 
			...
			<s:submit name="authmailer" value="Authorize" onclick="return anyComments()"/>
			<input type="button" name="deauthmaier" value="Reject" class="cancelbtn" onclick="return getReason()"/>
		</s:if>
		<s:else>
					NO VALUES FOUND
		</s:else>
