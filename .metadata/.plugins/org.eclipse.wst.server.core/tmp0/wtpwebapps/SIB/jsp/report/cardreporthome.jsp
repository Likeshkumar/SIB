...
/*  $('#actionhead').change(function() {
 ...
 function loadSubAction( actionhead ){
    ...
    if(actionhead == "ALL"){
  	...
  	var url = "getsubActionListMasterReportGeneration.do?actionhead="+actionhead;
 ...
 function validateForm(){
	  ...
	  var cardprocess = document.getElementById('cardprocess');
	 // alert(actionhead.value);
	  ...
	  if( cardprocess ){
		  if( cardprocess.value == "-1" ){  errMessage(cardprocess, "Select Card Action");  return false;  }
	  }
		...
		{
			errMessage(document.getElementById("fromdate"),"From date and to data should not be same");
			return false;
		    //alert("Error! ...");
 ...
 {     
	...
	if(validateForm()){ 
 	var url = "ExcelReportGenMasterMasterReportGeneration.do?RNO=3";
 	var result = AjaxReturnValue(url);
 	window.location = url;
	}
 }
...
<div align="center">
	<s:form action="generateCardReportMasterReportGeneration.do" autocomplete="off"  name="reportsgen" namespace="/">   
	...
	<tr>
			...
			<td>
			<select  id="cardprocess" name="cardprocess"   >
						<option value='-1'> Select </option> 
						<option value='INST'> INSTANT </option>     						        
						<option value='PERS'> PROCESS </option>    					
			</select>    
		...
		<tr id="actionrow" >
			...
			<td>
				...
				<tr>
					<td>
						<select  id="actionhead" name="actionhead" style='height:120px' multiple="multiple"  onclick='loadSubAction(this.options[this.selectedIndex].value)'>
						<!--  <option value='ALL'> ALL </option>-->
						<option value='REGN'> Registration </option>
						<option value='CARDGEN'> Card Generation </option>
						<option value='SEC'> CVV Data Generation </option>
						<option value='PRE'> Personalization </option>
						<!-- <option value='PINONLY'> Pin Data Generation </option> -->
						<option value='RECIEVE'> Receive </option>
						<option value='ISSUE'> Issuance </option>
						<option value='CUSTMAP'>Instant Issuance </option>
						</select>    
					</td>
					<td >   
						<div id="subhide" style="display: none;">	 
							<select  id="subaction"  name="subaction" style='width:250px' > 
