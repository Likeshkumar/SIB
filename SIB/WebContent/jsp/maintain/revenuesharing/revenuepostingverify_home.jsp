...
<script>
	...
	function showCommissionType( commissionfor ){
		...
		if( commissionfor !="$MC"){
			optionval += "<option value='$ALL'>ALL</option><option value='$EXP'>Expense</option> <option value='$REV'>Revenue</option>";
		}else{
...
<s:form action="revPostingVerifyListRevenueSharing.do"  name="orderform"  enctype="multipart/form-data"  onsubmit="return validateForm()"  autocomplete = "off"  namespace="/">
 
<table border="0" cellpadding="0" cellspacing="0" width="40%" class="formtable">	 
	 
	 <tr> <td> Bank Name </td> <s:hidden name="actioncode" id="actioncode" value="VERIFY"/>
	 <td> : <s:select name="bin" id="bin"  list="revbean.binlist" listKey="BIN" listValue="BIN_DESC" headerKey="-1" headerValue="-SELECT-" /> </td></tr>
	 
	 <tr> <td> Commission For </td>
	 <td> : <s:select name="commissionfor" id="commissionfor"  list="#{'$ALL':'ALL','$FLOAT':'Float','$MC':'Merchant'}"  onchange="showCommissionType(this.value)" headerKey="-1" headerValue="-SELECT-" /> </td></tr>
	 
	 <tr> <td> Commission Type </td>
	 <td> : <select name="commissiontype" id="commissiontype" > </select> </td></tr>
	 
	 <tr> <td> Currency </td>
	 <td> : <s:select name="curcode" id="curcode"  list="#{'834':'TZS'}" /> </td></tr>
