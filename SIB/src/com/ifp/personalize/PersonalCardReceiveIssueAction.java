 ...
 *
 */
public class PersonalCardReceiveIssueAction extends BaseAction {
...
*/
				...
				//cbs card link process
				/*int cbs = cbscardlink(order_refnum[i],instid,username,clearCardNumber);
				if(cbs == 0){
					continue;
				}else{
					continue;
	...
	private int cbscardlink(String order_refnum, String instid, String username, String clearCardNumber) {
		
		try {
			System.out.println(" Inside Active Card Report Post To CBS ==> ");
			trace(" Inside Active Card Report Post To CBS ==> ");
			
			String docu_qury = "select BRANCH_CODE,ORG_CHN,CIN,ACCT_NO,EMB_NAME,to_char(ISSUE_DATE,'yyyy-mm-dd') as ISSUE_DATE,"
					+ "ORDER_REF_NO,MCARD_NO,ORDER_REF_NO,to_char(EXPIRY_DATE,'yyyy-mm-dd') as EXPIRY_DATE,BULK_REG_ID "
					+ "from CARD_PRODUCTION WHERE INST_ID='"+instid+"' and org_chn='"+order_refnum+"'";
			System.out.println(docu_qury);
			trace(docu_qury);
			
			List carddetlist = jdbctemplate.queryForList(docu_qury);
			System.out.println("carddetlist===>"+carddetlist);
			String request = "";
			...
			while (custitr.hasNext()) {
				...
				request= "{ \"activeCards\":[{" +
				   		...
				   		"\"accountNumber\":\""+(String) mp.get("ACCT_NO")+"\"," +
			...
			String forwd = props.getProperty("cbs.api.fwd");
				...
				String cbslinkinsert = "INSERT INTO CBS_CARD_LINK(INSTID,CARDNO,MCARDNO,ACCT_NO,BRANCH,CIN,EMB_NAME,"
						...
						+ "ACCT_NO,BRANCH_CODE,CIN,EMB_NAME,ORDER_REF_NO,'"+username+"','"+json.getString("message")+"',"
								+ "'"+json.getString("responseCode")+"',SYSDATE,'P',BULK_REG_ID "
						+ "FROM CARD_PRODUCTION WHERE ORG_CHN='"+order_refnum+"'";
				System.out.println(cbslinkinsert);
				trace("===> CBS_CARD_LINK insert qry : "+cbslinkinsert);
				jdbctemplate.update(cbslinkinsert);
			} catch (Exception e) {
				e.printStackTrace();
				System.out.println("Exception in cbs card link");
				
				String cbslinkinsert = "INSERT INTO CBS_CARD_LINK(INSTID,CARDNO,MCARDNO,ACCT_NO,BRANCH,CIN,EMB_NAME,"
						+ "ORDER_REF_NO,UPLOAD_BY,UPLOAD_STATUS,UPLOAD_RESPCODE,ADDED_DATE,TYPE,CBS_BULK_REFID) SELECT INST_ID,ORG_CHN,MCARD_NO,"
						+ "ACCT_NO,BRANCH_CODE,CIN,EMB_NAME,ORDER_REF_NO,'"+username+"','"+e.getMessage()+"',"
								+ "'999',SYSDATE,'P',BULK_REG_ID "
						+ "FROM CARD_PRODUCTION WHERE ORG_CHN='"+order_refnum+"'";
				System.out.println(cbslinkinsert);
				trace("===> CBS_CARD_LINK insert qry : "+cbslinkinsert);
				jdbctemplate.update(cbslinkinsert);
		      ...
		      try {
		        // doing some work asynchronously ...
		    	  ...
		    	  PadssSecurity padsssec = new PadssSecurity();
					...
					String preinst_qury = "INSERT INTO CBS_CARD_LINK(INSTID,CARDNO,MCARDNO,ACCT_NO,BRANCH,CIN,EMB_NAME,"
		  					+ "ORDER_REF_NO,UPLOAD_BY,UPLOAD_STATUS,UPLOAD_RESPCODE,ADDED_DATE,TYPE,CBS_BULK_REFID) "
		  					+ "VALUES (?,?,?,?,?,?,?,?,?,?,?,SYSDATE,?,?)";
					insertpstmt = conn.prepareStatement(preinst_qury);
		  			System.out.println(preinst_qury);
		  			trace("===> CBS_CARD_LINK insert qry : "+preinst_qury);
		    	  ...
		    	  for (int i = 0; i < ordercount; i++) {
		  			...
		  			while (custitr.hasNext()) {
			 			...
			 			insertpstmt.setString(2, (String) mp.get("ORG_CHN"));
			 			insertpstmt.setString(3, (String) mp.get("MCARD_NO"));
						insertpstmt.setString(4, (String) mp.get("ACCOUNT_NO"));
						insertpstmt.setString(5, (String) mp.get("BRANCH_CODE"));
						insertpstmt.setString(6, (String) mp.get("CIN"));
						insertpstmt.setString(7, (String) mp.get("EMB_NAME"));
						insertpstmt.setString(8, (String) mp.get("ORDER_REF_NO"));
						insertpstmt.setString(13, (String) mp.get("BULK_REG_ID"));
		  			...
		  			insertpstmt.setString(9, username);
					insertpstmt.setString(10, json.getString("message"));
					insertpstmt.setString(11, json.getString("responseCode"));
					insertpstmt.setString(12, "P");
