...
public class CardMaintainAction extends BaseAction {
...
*/		System.out.println("change card status");
		...
		String card_status = "";
		String switchcardstatus = "", newcardno = null, productcode = "";
		...
		try {
			...
			String cin = "", embname = "", acctno = "", cardcollectbranch = "", CHECKER_ID = "", PRODUCT_CODE = "",prestatuscode="",
					binno = "";
			...
			if (!card_data.isEmpty()) {
				while (orderno.hasNext()) {
					...
					prestatuscode=(String)map.get("CARD_STATUS");

				}

			}
			trace("prestatuscode -->"+prestatuscode);
			...
			if (statuscode.equals("01")) {
			...
			}	else if ((statuscode.equals("05")) && (prestatuscode.equals("09"))) {
				...
				trace("card status changing as Normal from not activated");
				changedmsg = "Active";
				waitingmsg = " ";
			...
			else if(act.equals("M") && statuscode.equals("05")){
				System.out.println( "card status changing as Active");	
					

				System.out.println(" Active");
				 int checkvalidcard = cardmaindao.checkValidCardForActive( instid, enccardno,"M",statuscode, jdbctemplate );
			        
		         if(checkvalidcard ==1){
		        	 session.setAttribute("preverr", "E");
					 session.setAttribute("prevmsg", "Card Is   already Waiting for Active  Authorization ...");
					 return "required_home"; 
		         }
				int updatestatus=-1;
				auditbean.setAuditactcode("4107");
				if(padssenable.equals("Y")){
					updatestatus = cardmaindao.makerCardStatusActive(instid,padssenable, enccardno, statuscode, switchcardstatus, caf_recstatus, usercode, act,TABLENAME,collectbranch, jdbctemplate);
					}
				changedmsg = "Active and waiting for Checker Authorization";
				waitingmsg = ".";
				
			}
			else if (act.equals("C") && statuscode.equals("05")) 
			{
				System.out.println("card status changing as Active checker ");
				...
				waitingmsg = "";				
		         ...
		         if(checkvalidcard <1){
		        	 session.setAttribute("preverr", "E");
					 session.setAttribute("prevmsg", "Card Is   not Waiting for Active  Authorization ...");
					 return "required_home"; 
		         }else
		         {
		        	 System.out.println("go for checker authorization");
		        		int updatecardinfo = -1, updatestatus = -1;
						updatecardinfo = cardmaindao.updateezcardinfo(instid,hcardno,switchcardstatus, jdbctemplate);

						// updating cardproduction
						if (padssenable.equals("Y")) {
							updatestatus = cardmaindao.updateCardStatusforActive(instid,
									padssenable, enccardno, statuscode, switchcardstatus,
									caf_recstatus, usercode, TABLENAME, collectbranch,
									jdbctemplate);
						} else {
							updatestatus = cardmaindao.updateCardStatusforActive(instid,
									padssenable, cardno, statuscode, switchcardstatus,
									caf_recstatus, usercode, TABLENAME, collectbranch,
									jdbctemplate);
						}
						...
						if (updatestatus < 0 && updatecardinfo < 0) {
							...
							session.setAttribute("prevmsg",
									"Could not continue the process... unable to update status");
							return this.searchHome();
						}
						auditbean.setAuditactcode("4107");
						int inserthist = cardmaindao.insertMainintainHistory(instid,
								cardno, card_current_status, statuscode, usercode,
								jdbctemplate);
						if (inserthist < 0) {
							session.setAttribute("preverr", "E");
							session.setAttribute("prevmsg",
									"Could not continue the process... Could not insert history..");
							return this.searchHome();
						}
		         }		         			
			} 
				...
				System.out.println("card status changing as warm");
			...
			if (!statuscode.equals("04") && (!statuscode.equals("05"))) {
				// update ezcardinfo
				int updatecardinfo = -1, updatestatus = -1;
				updatecardinfo = cardmaindao.updateezcardinfo(instid, hcardno, switchcardstatus, jdbctemplate);
