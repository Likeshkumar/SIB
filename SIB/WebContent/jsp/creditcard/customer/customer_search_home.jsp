...
<script>
	...
	function searchCustomer(){
		...
		var qrystr = "customerSearchCreditCustRegisteration.do?doact="+doact.value;
...
<tr>
	...
	<td> <s:select list="%{creditbean.applicationlist}" id="applicationid"  name="applicationid" disabled="true"    headerKey="-1" headerValue="-SELECT-"  listKey="APPLICATIONID"  listValue="APPLICANT" value="%{custregbean.gender}"/> </td>
...
<form action='getcustomerDataCreditCustRegisteration.do' name='form1' id='form1'> 
...
<table border="0" cellpadding="0" cellspacing="4" width="20%" id="submitbtntable" style='display:none'>
		...
		<s:if test="%{creditbean.doact=='EDIT'}">
			<td> 
		...
		<s:elseif  test="%{creditbean.doact=='AUTH'}">
			<td> 
		...
		<s:elseif  test="%{creditbean.doact=='ALTER'}">
			<td> 
				<s:submit value="Submit" name="order" id="order" onclick="return validteCheckButton()"/>
