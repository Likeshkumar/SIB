...
<div align="center">
<s:form action="authPregenerationPreprocessAction" onsubmit="parent.showprocessing()" name="authrizeorder"  autocomplete = "off"  namespace="/">
	...
	<table width="90%" cellpadding="0"  border="0"  cellspacing="0"  class="formtable"  >
			<tr>
				...
				<s:iterator value="preauthorderlist" status="incr">
					<tr
					<% if (alt) { out.println( "class='alt'" );alt=false;}else{ alt=true;}  int rowcount = ++rowcnt; %>
					>
						<td> <%= rowcount %> </td>
						<td> <s:checkbox name="personalrefnum"  id="personalrefnum" fieldValue="%{ORG_CHN}"/>  </td>
						<td> ${ORDER_REF_NO}  </td>
						<td> ${MCARD_NO}  </td>
						<td> ${ACCT_NO}  </td>
						<td> ${EMB_NAME}  </td>
