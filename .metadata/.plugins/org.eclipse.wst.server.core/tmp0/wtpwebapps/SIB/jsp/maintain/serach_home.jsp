...
<%!
String maintdesc = null,act=null;
%>
<%
...
document.getElementById("title").innerHTML="Card Maintenance  "+"<% out.print(maintdesc);%>";
	...
	function searchRecords(){
		...
		{	 
		 ...
		 var url = "searchListCardMaintainAction.do?cardno="+cardno.value+"&selectedtype="+selectedtype.value+"&maintdesc="+maintdesc.value;
		 //alert(url);     
	...
	function showMaintain( instid, cardno,mcardno,cardnumber ){
		//alert(cardno); 
		//var a=test;
		//alert("cardnumber   "+cardno)
		var url = "showMaintainCardMaintainAction.do?instid="+instid+"&cardno="+cardno+"&mcardno="+mcardno+"&cardnumber="+cardnumber;
	...
	function enableField(selectedval)
	{
		...
		{
			//alert(selectedval);
			textcard.innerHTML="Enter Card Number";
			...
			//alert(selectedval);
			textcard.innerHTML="Enter Account Number";
			document.getElementById("cardno").value="";		
			
			selectedtype.value="acctnum";
			
		}
		else if (selectedval == "phonenoopt"){
			//alert(selectedval);
			...
			//alert(selectedval);
			textcard.innerHTML="Enter Customer Id";
			document.getElementById("cardno").value="";			
...
<s:form action="searchListCardMaintainAction.do" name="orderform" onsubmit="return searchRecords()" autocomplete="off" namespace="/">
...
<s:if test="%{#session.USERTYPE =='BRANCHUSER'}">
			...
			<td>  <s:property value="%{#session.BRANCH_DESC}"/></td>
				...
				<s:hidden name="mact" id="mact" value="%{mact}" />
	</s:if>

	 <%-- <input type="hidden" name="token" id="csrfToken"value="${csrfToken}"> --%>
	...
	<table border="0" cellpadding="0" cellspacing="4" width="40%" align="center">
		...
		<tr>
			...
			<!-- <td>
				<input type="radio" name="checkedtype" id="custnoopt"  onclick="enableField( this.value )" value="phonenoopt" /> Phone No 
			</td> -->
			...
			<td style="border: 1px solid #0099FF">
				<table border="0" cellpadding="0" cellspacing="4" width="40%"
					...
					<tr align="center">
						...
						<td><input type='text' name='cardno' id='cardno' maxlength="19" onkeypress="return numerals(event);"/>
