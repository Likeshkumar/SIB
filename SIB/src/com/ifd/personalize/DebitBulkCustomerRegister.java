...
public class DebitBulkCustomerRegister extends BaseAction{
...
{          
		...
		if(true){		    
			...
			///byg owtham
			String acctexistqry = "SELECT COUNT(1) AS CNT FROM ACCOUNTINFO WHERE ACCOUNTNO=? ";
			enctrace("acctexistqry-->"+acctexistqry);
			String acctexist = (String)jdbcTemplate.queryForObject(acctexistqry,new Object[]{pAccountNo}, String.class);
			...
			trace("Generated order reference number is : " + orderrefno);
			  //  System.out.println("Checking"+valBin+"|"+binno);
						...
						)
				{
					...
					getLimitType.append("select b.LIMIT_ID from PRODUCT_MASTER a, INSTPROD_DETAILS B "
					+ "where A.PRODUCT_CODE in "); 
					getLimitType.append("(select PRODUCT_CODE from INSTPROD_DETAILS where  "
					+ "SUB_PROD_ID='"+subProduct+"' AND INST_ID='"+instid+"') AND A.PRODUCT_CODE=B.PRODUCT_CODE "
					+ "and B.SUB_PROD_ID='"+subProduct+"'");
					...
					*/
					...
					+ "where A.PRODUCT_CODE in "); 
					getLimitType.append("(select PRODUCT_CODE from INSTPROD_DETAILS where  "
					+ "SUB_PROD_ID='"+subProduct+"' AND INST_ID=? ) AND A.PRODUCT_CODE=B.PRODUCT_CODE "
					+ "and B.SUB_PROD_ID=? ");
					getLimitType.append("		) and rownum=1 ");
					enctrace("limitlistqry :"+ getLimitType.toString());    
					String limitbasedon= (String) jdbcTemplate.queryForObject(getLimitType.toString(),new Object[]{
							subProduct,instid}, String.class);
					...
					String custexistqry = "SELECT COUNT(1) AS CNT FROM CUSTOMERINFO WHERE CIN=? ";
					enctrace("custexistqry-->"+custexistqry);
					String custexist = (String)jdbcTemplate.queryForObject(custexistqry, new Object[]{customerId},String.class);
...
}else
{
	...
	public String UploadFile() throws Exception
	{
		...
		trace("failename:::"+this.getUploadFileFileName());   
		String todaydate = commondesc.getDate("ddMMyyyy");      
		/*String upfile[] = getUploadFileFileName().replace(".txt","").split("_");
	 	if( !upfile[0].equals(todaydate) ){
		trace("Invalid file name....file name start with [ "+todaydate+" ] ");
		addActionError("Invalid file name....file name start with [ "+todaydate+ "]");
		return "DebitBulkRegHome";
	 	}
	 	else if( !(upfile[1].length()==3) ){
			trace("Serial No Should be 3 Digit [ "+todaydate+" ] ");
			addActionError("File Name Serial No Should be 3 Digit");
			return "DebitBulkRegHome";
			
		}else if (validateNumeric(upfile[1])==false)   
		{      
			addActionError("File Name Serial No Should be Numeric");
			return "DebitBulkRegHome";
		}
		else if (dao.getBulkfilenameCount(instid, this.getUploadFileFileName(), jdbctemplate)>0)
		{
			addActionError(getUploadFileFileName() +"- File name Already Exist Try With Differant Name");
	 		return "DebitBulkRegHome";
		}
	...
	*/
		...
		String walletdir = prop.getProperty("BULKREGISTRATION");   
		File walletuploadfile = new File( walletdir+"/"+todaydate);
		String origionalfilename = getUploadFileFileName();
		String failfilename = "FAIL_"+getUploadFileFileName();
		String successfilename = "SUCCESS_"+getUploadFileFileName();
		//trace("Got failfilename : " + failfilename  );
		
		
		
		if( getUploadFile() != null  ){
			File fileToCreate = new File(walletuploadfile, origionalfilename); 
			FileUtils.copyFile(getUploadFile(), fileToCreate);  
		
		File failfile = new File( walletdir+"/"+todaydate+"/"+failfilename );
		File successfile = new File( walletdir+"/"+todaydate+"/"+successfilename );
		
		FileWriter failwriter = new FileWriter(failfile);
		FileWriter successwriter = new FileWriter(successfile);
		     
		DebitBulkCustomerRegister scanner= new DebitBulkCustomerRegister();
		//String filename = getMyFile();
		System.out.println("this.getUploadFile()"+this.getUploadFile()+"this.getUploadFileFileName() \n  "+this.getUploadFileFileName());
		     
        int result = scanner.readFile(session,this.getUploadFile(),this.getUploadFileFileName(),successwriter,failwriter,usercode,jdbctemplate);    
        ...
        List statusList = dao.getUploadStatus(instid,this.getUploadFileFileName(),jdbctemplate);
