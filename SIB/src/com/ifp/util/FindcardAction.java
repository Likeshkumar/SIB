...
public class FindcardAction extends BaseAction
{
	...
	public void searchCard() throws Exception
	{
		...
		try
		{
			...
			///by gowtham300819
			String instantcount = "select count(*) from INST_CARD_PROCESS where INST_ID=? and "+ "CARD_NO = ?";
			//String instantstatus = "select * from INST_CARD_ORDER where INST_ID='"+instid+"' and ORDER_REF_NO like '%"+orderref+"%'";
			System.out.println("instantcount==> "+instantcount);
			int result_inst_qury = jdbctemplate.queryForInt(instantcount,new Object[]{instid,card_no});
			...
			//by gowtham300819
			String persoalizecount = "select count(*) from PERS_CARD_PROCESS where INST_ID=? and CARD_NO = ? ";
			//String instantstatus = "select * from INST_CARD_ORDER where INST_ID='"+instid+"' and ORDER_REF_NO like '%"+orderref+"%'";
			System.out.println("persoalizecount==> "+persoalizecount);
			int result_personalize_qury = jdbctemplate.queryForInt(persoalizecount,new Object[]{instid,card_no});
			System.out.println("result_personalize_qury==> "+result_personalize_qury);
			...
			if(card_no != ""&& !card_no.equals("null"))
			{
				if(result_inst_qury!=0)
				{
					...
					String cur_desc ="SELECT CUR_STATUS_DESC from IFP_CARD_STATUS_DESC "
					+ "where CARD_FLAG=? and STATUS_CODE =? ";
					carddesc=(String)jdbctemplate.queryForObject(cur_desc,new Object[]{"I",cardstatus},String.class);
				...
				/*	String cur_desc ="SELECT CUR_STATUS_DESC from IFP_CARD_STATUS_DESC where CARD_FLAG='I' and STATUS_CODE ='"+cardstatus+"'";
					...
					//by gowtham300819
					String cur_desc ="SELECT CUR_STATUS_DESC from IFP_CARD_STATUS_DESC where CARD_FLAG=? and STATUS_CODE =? ";
					carddesc=(String)jdbctemplate.queryForObject(cur_desc,new Object[]{"I",cardstatus},String.class);
...
*/		
	...
	//trace(acctno+"Card Number");
		...
		List<Map<String,Object>> list = jdbctemplate.queryForList(findcarddtlsqry);*/
				...
				if(!"0".equalsIgnoreCase(processcount))
				{
					findcarddtlsqry = "SELECT B.ORG_CHN,DECODE(A.ORDER_TYPE,'B','BULK REGISTRATION','P','NORMAL REGISTRATION') AS REGTYPE, B.CIN,B.MCARD_NO,NVL(B.PRE_FILE,'--') AS PREFILE,B.EMB_NAME,TO_CHAR(B.REG_DATE,'MM-DD-YY') AS REG_DATE,TO_CHAR(B.PRE_DATE,'DD-MM-YY') AS PRE_DATE,TO_CHAR(B.PIN_DATE,'DD-MM-YY') AS PIN_DATE,(SELECT CARD_TYPE_NAME FROM PRODUCT_MASTER WHERE PRODUCT_CODE=B.PRODUCT_CODE) AS PRODUCT, CASE WHEN B.CAF_REC_STATUS='S' THEN 'REISSUED' WHEN B.CAF_REC_STATUS='A' THEN 'NEW CARD' WHEN B.CAF_REC_STATUS='AC' THEN 'ADD ON CARD' WHEN B.CAF_REC_STATUS='DE' THEN 'DAMAGE WITH EXPIRY DATE' WHEN B.CAF_REC_STATUS='D' THEN 'DAMAGE' WHEN B.CAF_REC_STATUS='BN' THEN 'RENEWED' WHEN B.CAF_REC_STATUS='R' THEN 'REPIN' END AS ISSUEDTYPE, CASE WHEN B.CARD_STATUS='01' AND B.MKCK_STATUS='P' THEN 'Waiting For Security Data Generation' WHEN B.CARD_STATUS='01' AND B.MKCK_STATUS='M' THEN 'Waiting For Card Generation Authorization' WHEN B.CARD_STATUS='02' AND B.MKCK_STATUS='M' THEN 'Waiting For Security Data Authorization' WHEN B.CARD_STATUS='02' AND B.MKCK_STATUS='P' THEN 'Waiting For PRE Generation' WHEN B.CARD_STATUS='03' AND B.MKCK_STATUS='M' THEN 'Waiting For PRE Authorization' WHEN B.CARD_STATUS='03' AND B.MKCK_STATUS='P' THEN 'Waiting For Receive Card' WHEN B.CARD_STATUS='04' AND B.MKCK_STATUS='M' THEN 'Waiting For Receive Card Authorization' WHEN B.CARD_STATUS='04' AND B.MKCK_STATUS='P' THEN 'Waiting For Card Issuance' WHEN B.CARD_STATUS='05' AND B.MKCK_STATUS='M' THEN 'Waiting For Card Issuance Authorization' END AS CARD_STATUS FROM PERS_CARD_ORDER A,PERS_CARD_PROCESS B,CUSTOMERINFO C WHERE A.ORDER_REF_NO=B.ORDER_REF_NO AND b.CIN=c.CIN AND A.CIN=C.CIN AND B.ACCT_NO=? ";
				
				}
				else
				{
					findcarddtlsqry = "SELECT  B.ORG_CHN,'MIGRATED' AS REGTYPE,B.CIN,B.MCARD_NO,NVL(B.PRE_FILE,'--') AS PREFILE,B.EMB_NAME,TO_CHAR(B.REG_DATE,'MM-DD-YY') AS REG_DATE,TO_CHAR(B.PRE_DATE,'DD-MM-YY') AS PRE_DATE,TO_CHAR(B.PIN_DATE,'DD-MM-YY') AS PIN_DATE,(SELECT CARD_TYPE_NAME FROM PRODUCT_MASTER WHERE PRODUCT_CODE=B.PRODUCT_CODE) AS PRODUCT, CASE WHEN B.CAF_REC_STATUS='S' THEN 'REISSUED' WHEN B.CAF_REC_STATUS='A' THEN 'NEW CARD' WHEN B.CAF_REC_STATUS='AC' THEN 'ADD ON CARD' WHEN B.CAF_REC_STATUS='DE' THEN 'DAMAGE WITH EXPIRY DATE' WHEN B.CAF_REC_STATUS='D' THEN 'DAMAGE' WHEN B.CAF_REC_STATUS='BN' THEN 'RENEWED' WHEN B.CAF_REC_STATUS='R' THEN 'REPIN' END AS ISSUEDTYPE, CASE WHEN B.CARD_STATUS='01' AND B.MKCK_STATUS='P' THEN 'Waiting For Security Data Generation' WHEN B.CARD_STATUS='01' AND B.MKCK_STATUS='M' THEN 'Waiting For Card Generation Authorization' WHEN B.CARD_STATUS='02' AND B.MKCK_STATUS='M' THEN 'Waiting For Security Data Authorization' WHEN B.CARD_STATUS='02' AND B.MKCK_STATUS='P' THEN 'Waiting For PRE Generation' WHEN B.CARD_STATUS='03' AND B.MKCK_STATUS='M' THEN 'Waiting For PRE Authorization' WHEN B.CARD_STATUS='03' AND B.MKCK_STATUS='P' THEN 'Waiting For Receive Card' WHEN B.CARD_STATUS='04' AND B.MKCK_STATUS='M' THEN 'Waiting For Receive Card Authorization' WHEN B.CARD_STATUS='04' AND B.MKCK_STATUS='P' THEN 'Waiting For Card Issuance' WHEN B.CARD_STATUS='05' AND B.MKCK_STATUS='M' THEN 'Waiting For Card Issuance Authorization' END AS CARD_STATUS FROM PERS_CARD_PROCESS B,CUSTOMERINFO C WHERE b.CIN=c.CIN  AND B.ACCT_NO=? ";
				}
				enctrace("findcarddtlsqry--> "+findcarddtlsqry);
				System.out.println(findcarddtlsqry);
				List<Map<String,Object>> list = jdbctemplate.queryForList(findcarddtlsqry,new Object[]{acctno});
				
				
		for(int j=0;j<list.size();j++){
		/*	clearchn =  padsssec.getCHN(eDMK, eDPK, (String) list.get(j).get("CARD_NO"));*/
			clearchn=(String) list.get(j).get("MCARD_NO");
	...
	public String getCardIssudType(String instid, String value, String key, JdbcTemplate jdbctemplate ) throws Exception {
		...
		try{
			...
			///by gowtham300819
			String issuedtypeqry = " SELECT ORDER_FLAG FROM CARD_PRODUCTION WHERE INST_ID=? "+issuercond+" AND ROWNUM<=? ";
			enctrace("issuedtypeqry :"+issuedtypeqry);
			issuedtype = (String)jdbctemplate.queryForObject(issuedtypeqry,new Object[]{instid,"1"}, String.class);
	...
	public String getPreFile(String instid, String value, String key, JdbcTemplate jdbctemplate ) throws Exception {
		...
		try{
			...
			try{
				...
				//by gowtham300819
				prefilequery = "SELECT PRE_FILE FROM CARD_PRODUCTION  WHERE INST_ID=? "+precondition+" AND ROWNUM<=? ";
				enctrace("prefilequery1 : "+ prefilequery);
				prefile = (String)jdbctemplate.queryForObject(prefilequery,new Object[]{instid,"1"}, String.class); 
				...
				/*	prefilequery = "SELECT PRE_NAME FROM PERS_PRE_DATA_HIST  WHERE INST_ID='"+instid+"' "+precondition+" AND ROWNUM<=1";
					...
					///by gowtham300819
					prefilequery = "SELECT PRE_NAME FROM PERS_PRE_DATA  WHERE INST_ID=? "+precondition+" AND ROWNUM<=?";
					enctrace("prefilequery2 : "+ prefilequery);
					prefile = (String)jdbctemplate.queryForObject(prefilequery,new Object[]{instid,"1"}, String.class);
					...
					trace("Exception e2 :"+ e2.getMessage());
						try{
							...
							///by gowtham300819
							prefilequery = "SELECT PRE_NAME FROM PERS_PRE_DATA_HIST  WHERE INST_ID=? "+precondition+" AND ROWNUM<=?";
							enctrace("prefilequery3 : "+ prefilequery);
							prefile = (String)jdbctemplate.queryForObject(prefilequery, new Object[]{instid,"1"},String.class);
						   ...
						   try{
							...
							/*	prefilequery = "SELECT PRE_NAME FROM INST_PRE_DATA_HIST  WHERE INST_ID='"+instid+"' "+precondition+" AND ROWNUM<=1 ";
							   ...
							   ///by gowtham-300819
								prefilequery = "SELECT PRE_NAME FROM INST_PRE_DATA_HIST  WHERE INST_ID=? "+precondition+" AND ROWNUM<=? ";
								enctrace("prefilequery5 : "+ prefilequery);
								prefile = (String)jdbctemplate.queryForObject(prefilequery,new Object[]{instid,"1"}, String.class);
	...
	public String getorderstatusDesc(String orderstatus, JdbcTemplate jdbctemplate){
		...
		///by gowtham300819
				...
				enctrace (" cur_desc  : " + cur_desc );
				try{
				cardstus = (String) jdbctemplate.queryForObject(cur_desc,new Object[]{"I",orderstatus}, String.class);
	...
	public List getCardsFromOrder(String instid, String orderrefno,String tablename,String qury_card, JdbcTemplate jdbctemplate){ // order type is "INSTANT" OR "PERSONAL"
		...
		/*String query="select CARD_NO,CARD_STATUS,STATUS_CODE,ORDER_REF_NO, CARD_TYPE_ID, BIN, PRODUCT_CODE,"
		+ "  MKCK_STATUS, BRANCH_CODE,  EMB_NAME, CHECKER_ID, to_char(MAKER_DATE, 'dd-mon-yyyy') as "
		+ "ORDERED_DATE, MAKER_ID from "+tablename+" where INST_ID='"+instid+"' "+qury_card+"";
		chnlist = jdbctemplate.queryForList(query);*/
		
		///by gowtham-300819
				String query="select CARD_NO,CARD_STATUS,STATUS_CODE,ORDER_REF_NO, CARD_TYPE_ID, BIN, PRODUCT_CODE, "
				+ " MKCK_STATUS, BRANCH_CODE,  EMB_NAME, CHECKER_ID, to_char(MAKER_DATE, 'dd-mon-yyyy') as ORDERED_DATE, "
				+ "MAKER_ID from "+tablename+" where INST_ID=? "+qury_card+"";
	...
	public String getorderstatus(String instid,String tablename,String orderrefnum, JdbcTemplate jdbctemplate)
	{
		...
		///by gowtham-300819
				String order_status = "SELECT ORDER_STATUS from "+tablename+" where INST_ID=? and ORDER_REF_NO =? ";
				System.out.println("order_status changed tablename	---"+order_status);
				String orderstus = (String) jdbctemplate.queryForObject(order_status, new Object[]{instid,orderrefnum},String.class);
	...
	/*	String card_status = "select card_status from INST_CARD_PROCESS where INST_ID='"+instid+"' and ( CARD_NO = '"+card_no+"' OR ORG_CHN = '"+card_no+"')";
		...
		///by gowtham-30819
				String card_status = "select card_status from INST_CARD_PROCESS where INST_ID=? and ( CARD_NO =? OR ORG_CHN = ? )";
				card_inst = (String) jdbctemplate.queryForObject(card_status,new Object[]{instid,card_no,card_no},String.class);
	...
	public String cardstatus_personalize(String instid,String card_no,JdbcTemplate jdbctemplate)
	{
		...
		///by gowtham-290819
				String card_status = "select card_status from PERS_CARD_PROCESS where INST_ID=? and ( CARD_NO =? OR ORG_CHN =? )";
				card_personalize = (String) jdbctemplate.queryForObject(card_status,new Object[]{instid,card_no,card_no},String.class);
