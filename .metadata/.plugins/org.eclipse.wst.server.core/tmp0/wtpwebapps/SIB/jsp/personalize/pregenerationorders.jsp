	...
	if(!oneselected)
	{
		...
		if(type == 'E'){
			...
			if(check){
				...
				//document.authrizeorder.action = "genInstPREInstCardPREProcess.do?personalrefnum="+personalrefnum;
				
				document.authrizeorder.action = "preGenerationprocessPreprocessAction.do?personalrefnum="+personalrefnum;
				document.authrizeorder.submit();
...
<%-- <s:form action="genInstPREInstCardPREProcess"  name="authrizeorder"  autocomplete = "off"  namespace="/"> --%>
<s:form action="preGenerationprocessPreprocessAction" onsubmit="parent.showprocessing()" name="authrizeorder"  autocomplete = "off"  namespace="/">
	...
	<table width="90%" cellpadding="0"  border="0"  cellspacing="0"  class="formtable"  >
			...
			<tr>
				...
				<input type="hidden" name="Embosingname" value="${BRANCH_CODE}"/>
					...
					>
						<td> <%= rowcount %> </td>
						<td> <s:checkbox name="personalrefnum"  id="personalrefnum%{#incr.index}" fieldValue="%{ORDER_REF_NO}"/>  </td>
						<td> ${ORDER_REF_NO}  </td>
						<td> ${MCARD_NO}  </td>
						<td> ${ACCT_NO}  </td>
						<td> ${EMB_NAME}  </td>
